...
export async function generateCurrentMonthReport() {
  ...
  const curConv = (curMetrics.data || []).length
    ? (curMetrics.data || []).reduce((s, m) => s + Number(m.conversion_rate), 0) /
      (curMetrics.data || []).length
    : 0;
  const prevConv = (prevMetrics.data || []).length
    ? (prevMetrics.data || []).reduce((s, m) => s + Number(m.conversion_rate), 0) /
      (prevMetrics.data || []).length
    : 0;
  ...
  await generateSalesReport({
    ...
    revenue: {
      ...
      change: calcChange(curRevenue, prevRevenue),
    },
    sales: {
      current: curCompleted.length,
      previous: prevCompleted.length,
      change: calcChange(curCompleted.length, prevCompleted.length),
    },
