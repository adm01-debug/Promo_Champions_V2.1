...
describe("classifyPasswordError", () => {
  ...
  it("detects too-short password", () => {
    expect(classifyPasswordError({ message: "Password should be at least 8 characters" }).kind).toBe("too_short");
    expect(classifyPasswordError({ message: "Password too short" }).kind).toBe("too_short");
  });

  it("detects reused password", () => {
    const r = classifyPasswordError({ message: "New password should be different from the old password" });
    expect(r.kind).toBe("same_as_old");
  });

  it("detects rate limiting via status", () => {
    expect(classifyPasswordError({ message: "slow down", status: 429 }).kind).toBe("rate_limited");
    expect(classifyPasswordError({ message: "rate limit exceeded" }).kind).toBe("rate_limited");
  });
