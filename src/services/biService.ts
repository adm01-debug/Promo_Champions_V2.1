...
export const biService = {
  async getGestorData(): Promise<BIGestorData> {
    ...
    return {
      ...
      conversionByMonth: helpers
        .buildRevenueByMonth(last6MonthsSales)
        .map(r => ({ month: r.month, rate: 0 })),
  ...
  async getVendedorData(salesperson: {
    id: string;
    commission_rate: number;
  }): Promise<BIVendedorData> {
    const now = new Date();
    const monthStart = startOfMonth(now);
    const monthEnd = endOfMonth(now);
    const prevMonthStart = startOfMonth(subMonths(now, 1));
    ...
    return {
      ...
      recentAchievements: achievements
        .slice(0, 5)
        .map(a => ({ type: a.achievement_type, date: a.achievement_date })),
      ...
      conversationInsights: {
        ...
        buyingSignalsTotal: convAnalyses.reduce(
