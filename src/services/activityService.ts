...
export const activityService = {
  ...
  async getActivityStats(salespersonId?: string): Promise<ActivityStats> {
    ...
    let allQuery = supabase
      .from('activities')
      .select('activity_type, outcome, duration_minutes, created_at')
      .limit(2000);
    if (salespersonId) allQuery = allQuery.eq('salesperson_id', salespersonId);
    ...
    todayActivities.forEach(a => {
      if (a.activity_type in todayByType)
        todayByType[a.activity_type as keyof typeof todayByType]++;
