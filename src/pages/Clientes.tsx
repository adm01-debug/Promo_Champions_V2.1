      ...
      >
        <PageTransition>
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-border/10">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <p className="text-[10px] text-amber-500 font-bold uppercase tracking-wider">
                          {
                            Object.values(predictions).filter(p => p.urgency === 'high')
                              .length
                          }{' '}
                          EM JANELA CRÍTICA
                        </p>
                ...
                <div className="flex flex-wrap gap-3">
                  <AIEmailComposerButton
                    variant="outline"
                    className="h-11 px-5 rounded-xl border-primary/20 bg-primary/5 text-[10px] font-black uppercase tracking-widest hover:bg-primary/10 transition-all duration-300"
                  />
                  <CreateClientDialog />
                </div>
              </div>
                          ...
                          >
                            ...
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all duration-300 z-10">
                              <Button
                                variant="outline"
                                size="icon"
                                className="h-9 w-9 rounded-xl bg-background/50 border-border/50 hover:bg-primary/20 hover:text-primary"
                              ...
                              <Button
                                variant="outline"
                                size="icon"
                                className="h-9 w-9 rounded-xl bg-background/50 border-border/50 hover:bg-destructive/20 hover:text-destructive"
                            ...
                            <div className="flex items-start gap-4 mb-6">
                              <Avatar className="h-14 w-14 rounded-2xl ring-2 ring-background group-hover:ring-primary/20 transition-all">
                                <AvatarFallback className="bg-gradient-to-br from-primary/10 to-accent/10 text-primary font-black uppercase">
                                  {client.name
                                    .split(' ')
                                    .map(n => n[0])
                                    .join('')
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2">
                                  <p className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest truncate">
                                    {client.company || 'OPERATIVA INDEPENDENTE'}
                                  </p>
                            ...
                            {prediction && prediction.nextPurchaseDate && (
                              <div
                                className={cn(
                                  'mb-4 p-3 rounded-xl border flex items-center gap-3 animate-fade-in',
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <p className="text-xs font-bold">
                                    {prediction.daysToNextPurchase <= 0
                                      ? 'Expectativa Hoje!'
                                      : `Em ~${prediction.daysToNextPurchase} dias`}
                                  </p>
                                </div>
                                {prediction.riskOfChurn &&
                                  prediction.riskOfChurn > 30 && (
                            ...
                            <div className="space-y-3 mb-6">
                              {client.email && (
                                <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-accent/20 border border-white/5 group-hover:border-primary/10">
                              ...
                              <div className="flex justify-between items-end">
                                <div className="space-y-0.5">
                                  ...
                                  <div className="flex items-baseline gap-1">
                                    <TotalValueDisplay
                                      value={Number(client.total_value || 0)}
                                    />
                                ...
                                <div className="flex flex-col items-end gap-1">
                                  {prediction && prediction.confidence > 0.8 && (
                                    <Badge
                                      variant="outline"
                                      className="animate-pulse bg-primary/10 border-primary/30 text-primary text-[8px] font-black h-5 px-1.5"
                                    >
          ...
          <DeleteConfirmDialog
            ...
            description={`Confirma a exclusão de "${deletingClient?.name}"?`}
          ...
          <Dialog
            open={!!view360Client}
            onOpenChange={open => !open && setView360Client(null)}
          >
