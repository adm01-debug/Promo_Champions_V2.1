    ...
    return (
      <div className="space-y-3">
        {list.map(c => (
          <Card
            key={c.id}
            className="hover:shadow-md transition-shadow group border-white/5 bg-black/20 overflow-hidden"
          >
              ...
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div className="flex-1 min-w-[200px] ml-2">
                  <div className="flex items-center gap-2">
                    <p className="font-bold text-sm uppercase tracking-tight">
                      {c.sales?.client_name ?? 'Cliente'}
                    </p>
                    {c.is_first_sale && (
                      <Badge
                        variant="secondary"
                        className="bg-primary/20 text-primary border-primary/20 text-[8px] font-black uppercase tracking-tighter h-4 px-1"
                      >
                        Ativação
                      </Badge>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground truncate uppercase font-medium">
                    {c.sales?.product_name ?? '—'}
                  </p>
                ...
                <div className="text-right">
                  ...
                  <Badge
                    variant="outline"
                    className={cn(
                      'text-[9px] font-black uppercase tracking-widest px-2 py-0.5 mt-1 border-white/10',
                      c.status === 'paid'
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Acompanhe suas comissões pendentes, aprovadas e pagas em tempo real."
        />
      </Helmet>
      ...
      <main className="container max-w-7xl py-6 space-y-8 animate-in fade-in duration-500">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground uppercase tracking-widest font-bold">
              Relatório Financeiro de Performance
            </p>
          </div>
        ...
        >
          {[
            ...
            <Card
              key={i}
              className="border-white/5 bg-black/40 backdrop-blur-md overflow-hidden relative group"
            >
              <div
                className={cn(
                  'absolute top-0 right-0 w-24 h-24 blur-[40px] rounded-full opacity-10 transition-opacity group-hover:opacity-20',
                  item.bg
                )}
              />
            ...
            <Card className="border-dashed border-white/10 bg-transparent">
              <CardContent className="p-4 space-y-3">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed italic">
                  As comissões são geradas automaticamente após a conclusão da venda e
                  auditadas pelo departamento financeiro em até 48h.
                </p>
              </CardContent>
            </Card>
          </div>
