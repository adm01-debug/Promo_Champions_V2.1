  ...
  return (
    ...
    >
      ...
      <div className="flex items-baseline gap-2">
        ...
        >
          {isHero && (
            <span className="text-2xl sm:text-3xl mr-1 text-muted-foreground/50 font-medium">
              R$
            </span>
          )}
          {value}
...
const VendedorDashboard = () => {
  ...
  const { salesperson, goal, currentSales, previousSales, allSales } = data || {
    ...
    goal: 0,
    currentSales: [] as Sale[],
    previousSales: [] as Sale[],
    allSales: [] as Sale[],
  };
  ...
  const pendingSales = currentSales.filter(
    s => s.status === 'pending' || s.status === 'negotiation'
  );
  ...
  const staleTasks = pendingSales.filter(
    sale => differenceInDays(new Date(), parseISO(sale.created_at)) >= 3
  );
        ...
        <div className="min-h-screen bg-background p-6 lg:p-8">
          <div className="max-w-[1400px] mx-auto space-y-6">
              ...
              {[
                {
                  ...
                  value: animatedRevenue.toLocaleString('pt-BR'),
            ...
            {dailyRequired > 0 && goalProgress < 100 && (
              <div
                className="opacity-0 animate-fade-in-up glass rounded-xl p-4 border border-warning/30 bg-warning/5"
