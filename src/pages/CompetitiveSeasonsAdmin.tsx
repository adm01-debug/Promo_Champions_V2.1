...
import { Flame, Plus, Calendar, Zap, Clock, RotateCcw } from 'lucide-react';
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Gerencie temporadas competitivas e multiplicadores de XP."
        />
      </Helmet>
      <PageTransition>
          ...
          >
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Crie e gerencie temporadas com bônus de XP
              </p>
            </div>
            <div className="flex gap-2">
              <Button
                ...
                onClick={async () => {
                  if (
                    confirm(
                      'Deseja realmente resetar as ligas e o ranking semanal agora? Esta ação não pode ser desfeita.'
                    )
                  ) {
                    ...
                    if (error) toast.error('Erro ao resetar arena: ' + error.message);
                      ...
                      toast.success(
                        'Arena resetada com sucesso! Todos os placares semanais foram zerados.'
                      );
                  }
                }}
              >
            ...
            <motion.div variants={itemVariants}>
              <Card className="p-4 glass border-border/40 space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <Input
                    placeholder="Nome (ex: Temporada de Verão)"
                    value={name}
                    onChange={e => setName(e.target.value)}
                  />
                  <Input
                    placeholder="Multiplicador XP"
                    type="number"
                    step="0.1"
                    value={xpMultiplier}
              ...
              {seasons.map(season => {
                ...
                return (
                  <Card
                    key={season.id}
                    className={cn(
                      'p-4 glass border-border/40',
                      season.status === 'active' && 'border-primary/30'
                    )}
                  >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 text-xs text-muted-foreground">
                          <span className="flex items-center gap-1">
                            ...
                            {format(new Date(season.starts_at), 'dd/MM/yyyy', {
                              locale: ptBR,
                            })}
                          </span>
                          <span>→</span>
                          <span>
                            {format(new Date(season.ends_at), 'dd/MM/yyyy', {
                              locale: ptBR,
                            })}
                          </span>
                          {season.status === 'active' && (
                            <span className="flex items-center gap-1 text-primary">
                              ...
                              Encerra{' '}
                              {formatDistanceToNow(new Date(season.ends_at), {
                                addSuffix: true,
                                locale: ptBR,
                              })}
                            </span>
                      ...
                      <div className="flex gap-1">
                        {season.status === 'upcoming' && (
                          <Button
                            ...
                            className="text-xs"
                            onClick={() =>
                              statusMutation.mutate({ id: season.id, status: 'active' })
                            }
                          >
                            Ativar
                          </Button>
