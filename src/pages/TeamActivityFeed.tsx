  ...
  const { data: activities, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('activities')
        .select(
          'id, activity_type, outcome, notes, contact_name, created_at, duration_minutes, salesperson_id, salespeople:salesperson_id(name, avatar_url)'
        )
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Acompanhe as atividades recentes de toda a equipe em tempo real."
        />
      </Helmet>
      <PageTransition>
        <div className="container max-w-3xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
          <motion.div variants={itemVariants}>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Atividades recentes de todos os vendedores
            </p>
          </motion.div>
                  ...
                  return (
                    <motion.div key={act.id} variants={itemVariants}>
                      <div className="relative flex items-start gap-4 py-3 pl-2">
                        {/* Timeline dot */}
                        <div
                          className={cn('relative z-10 p-2 rounded-xl shrink-0', cfg.bg)}
                        >
