  ...
  const createApprovalMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: 'Alteração aguardando aprovação.',
      });
    },
  });
    ...
    mutationFn: async ({
      ...
      if (status === 'approved') {
        ...
        if (request.type === 'goal') {
          ...
          const values = request.new_values as {
            weight: number;
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-border/10 pb-6">
          <div>
            ...
            <p className="text-muted-foreground mt-1 uppercase text-[10px] font-bold tracking-widest">
              Configuração de Metas, Pontuações e Comissões
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              ...
              <Input
                ...
                className="w-44 bg-background/50 border-border/40 h-9"
              />
            </div>
            <Badge
              variant="outline"
              className="bg-primary/5 border-primary/20 text-primary uppercase text-[10px] py-2 px-3 self-end"
            >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="bg-muted/30 p-1 border border-border/40 w-full md:w-auto h-auto">
            ...
            <TabsTrigger value="pontuacao">
              <Award className="h-4 w-4 mr-2" /> Regras
            </TabsTrigger>
                        ...
                        <Input
                          ...
                          onBlur={e =>
                            createApprovalMutation.mutate({
                              ...
                              newValues: {
                                weight: rule.weight,
                                points_per_unit: parseFloat(e.target.value),
                                label: rule.label,
                              },
                              oldValues: { points_per_unit: rule.points_per_unit },
                            })
                          }
                        />
                      </div>
                        ...
                        <TableRow key={sp.id}>
                          ...
                          <TableCell className="text-center">
                            <Input
                              type="number"
                              step="0.1"
                              defaultValue={config?.rate || 0}
                              className="w-24 mx-auto text-center"
                              onBlur={e =>
                                createApprovalMutation.mutate({
                                  type: 'commission',
                                  entityId: sp.id,
                                  newValues: { rate: parseFloat(e.target.value) },
                                  oldValues: { rate: config?.rate || 0 },
                                })
                              }
                            />
                          </TableCell>
              ...
              {approvalRequests?.filter(r => r.status === 'pending').length === 0 ? (
                <Card className="glass border-border/40 py-12">
                  <CardContent className="flex flex-col items-center justify-center text-center">
                    ...
                    <CardDescription>
                      Não há solicitações de alteração pendentes para revisão.
                    </CardDescription>
                  </CardContent>
                </Card>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {approvalRequests
                    ...
                    .map(req => (
                      <Card
                        key={req.id}
                        className="glass border-border/40 overflow-hidden group hover:border-primary/40 transition-all duration-300"
                      >
                        <div className="bg-muted/30 px-4 py-2 border-b border-border/40 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold text-muted-foreground uppercase">
                              {req.competence_month
                                ? format(new Date(req.competence_month), 'MMMM yyyy', {
                                    locale: ptBR,
                                  })
                                : '-'}
                            </span>
                      ...
                      <TableBody>
                        {approvalRequests
                          ...
                          .map(req => (
                            ...
                            >
                              <TableCell className="text-[10px] font-bold uppercase italic">
                                {req.type}
                              </TableCell>
                              <TableCell className="text-[10px]">
                                {req.competence_month
                                  ? format(new Date(req.competence_month), 'MM/yyyy')
                                  : '-'}
                              </TableCell>
                              <TableCell>
                                <Badge
              ...
              <CardHeader className="flex flex-row items-center justify-between border-b border-border/10">
                <div>
                  ...
                  <CardDescription className="text-[10px] uppercase">
                    Rastreabilidade completa de todas as alterações comerciais
                  </CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-8 text-[10px] font-bold uppercase border-border/40"
                  >
                ...
                <Table>
                  <TableHeader className="bg-muted/20 sticky top-0 z-10">
                    <TableRow className="hover:bg-transparent border-border/10">
                      <TableHead className="w-[150px] text-[10px] font-black uppercase">
                        Data/Hora
                      </TableHead>
                  ...
                  <TableBody>
                    {auditLogs?.map(log => (
                      <TableRow
                        key={log.id}
                        className="hover:bg-primary/5 transition-colors border-border/10"
                      >
                        ...
                        <TableCell>
                          ...
                          >
                            {log.action
                              .replace('approved_', 'APROVADO: ')
                              .replace('create_approval_', 'SOLICITADO: ')}
                          </Badge>
                        </TableCell>
                        ...
                        <TableCell>
                          <TooltipProvider>
                            <Tooltip>
                              ...
                              <TooltipContent
                                side="left"
                                className="w-80 p-3 bg-black/95 border-primary/30"
                              >
