...
import { MessageSquare, Send, ThumbsUp, ThumbsDown, Minus } from 'lucide-react';
...
const NPSGauge = React.memo(({ nps }: { nps: number }) => {
  const color =
    nps >= 50 ? 'text-green-500' : nps >= 0 ? 'text-yellow-500' : 'text-destructive';
      ...
      <PageTransition>
        <div className="space-y-6">
          ...
          <div className="glass rounded-xl p-4 border border-border/40 flex gap-2">
            ...
            <Button
              onClick={handleSend}
              disabled={!newClient.trim() || createSurvey.isPending}
              size="sm"
            >
                    ...
                    {survey.status !== 'responded' && (
                      <Dialog>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            <DialogTitle>
                              Registrar Resposta — {survey.client_name}
                            </DialogTitle>
                          </DialogHeader>
