  ...
  const { data: clients, isLoading: clientsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('clients')
        .select('id, name, company, total_value, updated_at, created_at');
      ...
      .map(client => {
        ...
        const lastActivity =
          clientSales.length > 0
            ? Math.min(
                ...clientSales.map(s =>
                  differenceInDays(new Date(), new Date(s.created_at))
                )
              )
            : 999;
        const recencyScore =
          lastActivity < 7 ? 30 : lastActivity < 30 ? 20 : lastActivity < 90 ? 10 : 0;
        ...
        const tier: ClientHealth['tier'] =
          totalScore >= 60 ? 'healthy' : totalScore >= 30 ? 'at_risk' : 'critical';
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Gestão completa de pós-venda, renovações, tickets e saúde do cliente."
        />
      </Helmet>
      <PageTransition>
        <div className="container max-w-6xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
          ...
          >
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Visão holística da saúde, renovações e engajamento da base.
              </p>
            </div>
            <div className="flex gap-2">
              <Badge
                variant="outline"
                className="bg-status-success/5 text-status-success border-status-success/30"
              >
                {summary.healthy} Saudáveis
              </Badge>
              <Badge
                variant="outline"
                className="bg-status-warning/5 text-status-warning border-status-warning/30"
              >
                {summary.atRisk} Em Risco
              </Badge>
              <Badge
                variant="outline"
                className="bg-destructive/5 text-destructive border-destructive/30"
              >
          ...
          <Tabs defaultValue="health" className="space-y-6">
            <TabsList className="grid grid-cols-2 md:grid-cols-4 w-full md:w-[600px] h-auto p-1 bg-muted/50 rounded-xl">
              <TabsTrigger value="health" className="rounded-lg py-2">
                Health Score
              </TabsTrigger>
              <TabsTrigger value="renewals" className="rounded-lg py-2">
                Renovações
              </TabsTrigger>
              <TabsTrigger value="usage" className="rounded-lg py-2">
                Adoção (Uso)
              </TabsTrigger>
              <TabsTrigger value="support" className="rounded-lg py-2">
                Tickets
              </TabsTrigger>
            </TabsList>
              ...
              {clientsLoading ? (
                ...
                <Card className="p-12 text-center glass border-border/40">
                  ...
                  <p className="text-muted-foreground">
                    Nenhum dado de saúde disponível ainda.
                  </p>
                </Card>
              ) : (
                  ...
                  {healthData.map(client => {
                    ...
                    return (
                      <motion.div key={client.id} variants={itemVariants}>
                        <Card
                          className={cn(
                            'p-4 glass border-border/40 hover:border-primary/20 transition-all group'
                          )}
                        >
                          <div className="flex items-center gap-4">
                            <div
                              className={cn(
                                'relative w-14 h-14 rounded-full flex items-center justify-center shrink-0 shadow-inner',
                                cfg.bg
                              )}
                            >
                              <span
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                <Badge
                                  variant="outline"
                                  className={cn(
                                    'text-[9px] font-bold h-4 px-1.5 uppercase',
                                    cfg.color,
                                    cfg.border
                                  )}
                                >
  ...
  return (
    <div className="grid gap-4">
      ...
      {renewals?.map(ren => (
        <Card
          key={ren.id}
          className="p-4 flex items-center justify-between hover:bg-muted/30 transition-colors border-l-4 border-l-primary"
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                ...
                Expira em{' '}
                {ren.contract_end_date
                  ? format(parseISO(ren.contract_end_date), "dd 'de' MMM, yyyy", {
                      locale: ptBR,
                    })
                  : 'N/A'}
              </div>
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
              <p className="text-sm font-bold">
                R$ {ren.renewal_value?.toLocaleString()}
              </p>
              <Badge
                variant={
                  ren.risk_level === 'high'
                    ? 'destructive'
                    : ren.risk_level === 'medium'
                      ? 'secondary'
                      : 'outline'
                }
            ...
            <div className="w-16">
              <p className="text-[10px] text-muted-foreground text-center mb-1 font-bold">
                {ren.probability}% Prob.
              </p>
              <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                <div
                  className="h-full bg-primary"
                  style={{ width: `${ren.probability}%` }}
                />
              </div>
            </div>
          </div>
  ...
  return (
    <div className="grid gap-3">
      {tickets?.map(t => (
        <Card
          key={t.id}
          className="p-4 hover:bg-muted/20 transition-all border-l-4 border-l-amber-500"
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <span className="text-[10px] text-muted-foreground">
                {t.created_at ? format(parseISO(t.created_at), 'dd/MM HH:mm') : '—'}
              </span>
            </div>
          </div>
        </Card>
