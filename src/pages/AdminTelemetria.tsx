...
export default function AdminTelemetriaPage() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const headers = ["Data/Hora", "Operação", "Tabela/RPC", "Duração", "Severidade", "Records", "Limit", "Erro"];
                  ...
                  <tbody>
                    {rows.map((row) => (
                      <tr key={row.id} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                        ...
                        <td className="p-3 text-xs text-muted-foreground max-w-[200px] truncate" title={row.error_message || ""}>
                          {row.error_message || "-"}
                        </td>
                      </tr>
                    ))}
