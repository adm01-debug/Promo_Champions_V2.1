...
import {
  ...
  useCircuitBreakerHistory,
} from '@/hooks/useCircuitBreakerHistory';
import {
  ShieldAlert,
  ShieldCheck,
  LogIn,
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Dashboard de segurança com IPs, logins e circuit breakers."
        />
      </Helmet>
      <PageTransition>
        <div className="container max-w-6xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
          <motion.div variants={itemVariants}>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Monitoramento de IPs, logins e circuit breakers
            </p>
          </motion.div>
            ...
            stats && (
              ...
              >
                ...
                <Card className="p-4 glass border-border/40 text-center">
                  ...
                  <p className="text-2xl font-display font-bold">
                    {stats.blockedRequests24h}
                  </p>
                  <p className="text-[10px] text-muted-foreground">
                    Requests Bloqueados (24h)
                  </p>
                </Card>
              </motion.div>
            )
          ...
          <motion.div variants={itemVariants}>
            <Card className="glass border-border/40 overflow-hidden">
              <div className="p-4 border-b border-border/30 flex items-center justify-between">
                ...
                <Badge
                  variant="outline"
                  className="bg-primary/10 text-primary border-primary/20 text-[10px] font-black uppercase tracking-widest px-4"
                >
                  Active Shields
                </Badge>
              </div>
                  ...
                  <div className="space-y-4">
                    <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed mb-4">
                        O bloqueio geográfico impede qualquer requisição vinda de países
                        marcados como suspeitos ou fora da sua zona de operação.
                      </p>
                    ...
                    <Button
                      variant="outline"
                      className="w-full h-12 border-primary/20 bg-primary/10 text-primary hover:bg-primary/20 text-xs font-black uppercase tracking-widest rounded-xl transition-all"
                    >
                      Ver Logs Detalhados
                    </Button>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <motion.div variants={itemVariants}>
              <Card className="glass border-border/40">
                <div className="p-4 border-b border-border/30">
                  <h3 className="font-display font-semibold text-sm flex items-center gap-2">
                    <LogIn className="h-4 w-4 text-primary" /> Tentativas de Login
                    Recentes
                  </h3>
                </div>
                <div className="p-4 space-y-2 max-h-[400px] overflow-y-auto">
                  {loginsLoading ? (
                    ...
                    loginAttempts.map(attempt => (
                      <div
                        key={attempt.id}
                        className="flex items-center gap-3 py-1.5 px-2 rounded-lg hover:bg-muted/30 text-xs"
                      >
                        {attempt.success ? (
                        ...
                        <span className="text-muted-foreground shrink-0">
                          {formatDistanceToNow(
                            new Date(attempt.created_at ?? new Date()),
            ...
            <motion.div variants={itemVariants}>
              <Card className="glass border-border/40">
                <div className="p-4 border-b border-border/30">
                  <h3 className="font-display font-semibold text-sm flex items-center gap-2">
                    <Activity className="h-4 w-4 text-status-warning" /> Circuit Breaker
                    Events
                  </h3>
                </div>
                <div className="p-4 space-y-2 max-h-[400px] overflow-y-auto">
