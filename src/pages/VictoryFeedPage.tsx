  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Celebre conquistas e vitórias da equipe de vendas."
        />
      </Helmet>
      <PageTransition>
          ...
          >
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Celebrações e conquistas em tempo real
              </p>
            </div>
          </motion.div>
          ...
          {isLoading ? (
            ...
            <Card className="p-8 text-center glass border-border/40">
              ...
              <p className="text-sm text-muted-foreground">
                As conquistas aparecerão aqui automaticamente!
              </p>
            </Card>
          ) : (
            <motion.div variants={itemVariants} className="space-y-4">
              {feedItems.map(item => {
                ...
                return (
                  <Card
                    key={item.id}
                    className="glass border-border/40 overflow-hidden group"
                  >
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          >
                            ...
                            <Badge
                              variant="secondary"
                              className="text-[9px] font-black uppercase tracking-widest px-1.5 h-4"
                            >
                        ...
                        <div
                          className={cn(
                            'p-2 rounded-xl bg-primary/10 border border-primary/20 shadow-inner',
                            eventConfig.color
                          )}
                        >
                      ...
                      <div className="flex items-center gap-1 flex-wrap pt-2 border-t border-border/20">
                        {REACTION_EMOJIS.map(emoji => {
                          const count = reactions.filter(
                            (r: { reaction: string }) => r.reaction === emoji
                          ).length;
                          return (
                            <Button
                              key={emoji}
                          ...
                          <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                            {comments.map(
                              (comment: {
                                ...
                                <div
                                  key={comment.id}
                                  className="flex gap-2 text-xs bg-muted/30 p-2.5 rounded-xl"
                                >
                                  <span className="font-bold text-primary min-w-fit">
                                    {comment.salespeople?.name || 'Anônimo'}:
                                  </span>
