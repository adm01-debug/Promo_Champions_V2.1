...
export default function HistoricoDesafiosDiarios() {
  ...
  const { data: history, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const thirtyDaysAgo = getLocalISODate(subDays(new Date(), 30));
  ...
  function calculateStreak(
    dates: string[],
    grouped: Record<string, DailyChallengeWithProgress[]>
  ): number {
    let streak = 0;
      ...
      <div className="min-h-screen bg-background p-6 lg:p-8 space-y-8">
        ...
        >
          <div>
            <h1 className="text-page-title flex items-center gap-3">
              <Zap className="h-8 w-8 text-rank-gold" />
        ...
        >
          {statCards.map(({ icon: Icon, color, label, value }) => (
            <motion.div key={label} variants={itemVariants}>
              <Card
                className={`bg-gradient-to-br from-${color}-500/10 to-${color}-600/5 border-${color}-500/20`}
              >
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`h-12 w-12 rounded-xl bg-${color}-500/20 flex items-center justify-center`}
                    >
        ...
        {salesperson?.id && (
          ...
          >
            ...
            >
              <CheckCircle
                className={`h-4 w-4 ${checkStreakMilestone.isPending ? 'animate-spin' : ''}`}
              />
