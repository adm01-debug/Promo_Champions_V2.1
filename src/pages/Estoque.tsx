...
function StockStatusBadge({
  ...
  if (current <= reorder)
    return (
      <Badge className="bg-amber-500/20 text-amber-500 border-none font-black uppercase tracking-widest text-[10px]">
        LOW STOCK
      </Badge>
    );
  return (
    <Badge className="bg-emerald-500/10 text-emerald-500 border-none font-black uppercase tracking-widest text-[10px]">
      OPTIMAL
    </Badge>
  );
}
...
export default function Estoque() {
  ...
  const lowCount = inventory.filter(
    i => i.current_stock > i.min_stock_level && i.current_stock <= i.reorder_point
  ).length;
        ...
        <div className="space-y-6 p-4 md:p-6">
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={v => setForm(f => ({ ...f, movement_type: v }))}
                      >
                        <SelectTrigger className="h-12 bg-muted/20 border-border/40 font-bold">
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        type="number"
                        placeholder="0"
                        className="h-12 bg-muted/20 border-border/40 text-sm font-bold"
                        value={form.quantity}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="h-12 bg-muted/20 border-border/40 text-sm font-medium italic"
                      value={form.reason}
                  ...
                  <Button
                    ...
                    className="w-full bg-primary text-[10px] font-black uppercase tracking-widest h-12 rounded-xl mt-2"
                  >
            ...
            <Card className="relative overflow-hidden bg-gradient-to-br from-card/80 to-card/40 border-none shadow-2xl backdrop-blur-md group">
              <CardContent className="p-6 flex items-center gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-baseline gap-1.5">
                    <span className="font-display font-black text-3xl tracking-tighter text-rose-500">
                      {criticalCount}
                    </span>
            ...
            <Card className="relative overflow-hidden bg-gradient-to-br from-card/80 to-card/40 border-none shadow-2xl backdrop-blur-md group">
              <CardContent className="p-6 flex items-center gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-baseline gap-1.5">
                    <span className="font-display font-black text-3xl tracking-tighter text-amber-500">
                      {lowCount}
                    </span>
            ...
            <Card className="relative overflow-hidden bg-gradient-to-br from-card/80 to-card/40 border-none shadow-2xl backdrop-blur-md group">
              <CardContent className="p-6 flex items-center gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-baseline gap-1.5">
                    <span className="font-display font-black text-3xl tracking-tighter text-emerald-500">
                      {entriesCount}
                    </span>
          ...
          <Tabs defaultValue="inventory" className="w-full">
            <TabsList className="bg-muted/20 border border-border/10 p-1 rounded-xl h-12">
              <TabsTrigger
                value="inventory"
                className="rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground text-[10px] font-black uppercase tracking-widest px-6 h-full"
              >
                  ...
                  {loadingInv ? (
                    ...
                    <div className="p-12 text-center flex flex-col items-center">
                      ...
                      <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">
                        Aguardando suprimentos da elite...
                      </p>
                    </div>
                  ) : (
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-border/10 text-left bg-muted/20">
                            ...
                            <th className="p-4 text-[10px] font-black text-muted-foreground uppercase tracking-widest text-right">
                              Last Sync
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-border/10">
                          {inventory.map(item => {
                            const pct = Math.min(
                              100,
                              (item.current_stock / (item.max_stock_level || 100)) * 100
                            );
                            return (
                              <tr
                                key={item.id}
                              ...
                              >
                                <td className="p-4">
                                  <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest">
                                      SKU:{' '}
                                      {(item.products as { sku?: string })?.sku || '—'}
                                    </span>
                                  </div>
                                </td>
                                <td className="p-4">
                                    ...
                                    <div className="h-2 w-full bg-muted/20 rounded-full overflow-hidden relative">
                                      ...
                                      {item.reorder_point && item.max_stock_level && (
                                        <div
                                          ...
                                          style={{
                                            left: `${(item.reorder_point / item.max_stock_level) * 100}%`,
                                          }}
                                ...
                                <td className="p-4 flex items-center gap-3">
                                  ...
                                  />
                                  <div className="flex flex-col">
                                    <span className="text-[9px] font-black text-muted-foreground/40 uppercase leading-none">
                                      Min: {item.min_stock_level}
                                    </span>
                                    <span className="text-[9px] font-black text-primary/40 uppercase leading-none mt-1">
                                      Reorder: {item.reorder_point}
                                    </span>
                                  </div>
                                </td>
                                <td className="p-4 text-right">
                                  <span className="text-[10px] font-black text-muted-foreground uppercase tabular-nums">
                                    {item.last_restock_date
                                      ? format(
                                          new Date(item.last_restock_date),
                                          'dd/MM/yy',
                                          { locale: ptBR }
                                        )
                                      : 'NEVER'}
                ...
                <AnimatePresence>
                  {loadingForecast ? (
                    ...
                    <div className="col-span-full p-12 text-center bg-card/40 rounded-2xl border border-dashed border-border/50">
                      ...
                      <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">
                        Aguardando telemetria de vendas para processar previsões...
                      </p>
                    </div>
                  ) : (
                            ...
                            <div className="flex justify-between items-end">
                              <div>
                                ...
                                <p className="font-display font-black text-xl">
                                  ...
                                  <span className="text-[10px] italic">UN/DAY</span>
                                </p>
                              </div>
                            ...
                            {item.expectedOutOfStockDate && (
                              <div className="flex items-center gap-2 mt-4 p-2 rounded-lg bg-black/20 border border-white/5">
                                ...
                                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                                  Expected Zero-Day:{' '}
                                  {format(item.expectedOutOfStockDate, 'dd/MM/yyyy')}
                                </p>
                              </div>
                            )}
                          </CardContent>
                ...
                <CardContent className="p-0">
                  {loadingMov ? (
                    ...
                    <div className="p-12 text-center flex flex-col items-center">
                      ...
                      <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">
                        No strategic movements recorded in the tactical logs.
                      </p>
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                            ...
                            >
                              ...
                              <td className="p-4">
                                <Badge
                                  className={cn(
                                    'text-[10px] font-black uppercase tracking-widest',
                                    move.movement_type === 'entry'
                                      ? 'bg-emerald-500/20 text-emerald-500 border-emerald-500/30'
                                      : move.movement_type === 'exit'
