...
const ROLES: {
  ...
  {
    role: 'closer',
    title: 'Corrida dos Closers',
    emoji: '🎯',
    description:
      'Disputa entre fechadores de vendas. Pontuação baseada em valor, markup e novos clientes.',
    gradient: 'from-primary/20 via-primary/5 to-transparent',
    href: '/race-arena/closer',
    Icon: Handshake,
  },
  {
    role: 'sdr',
    title: 'Corrida dos SDRs',
    emoji: '📞',
    description:
      'Disputa de prospecção. Pontuação por stakeholders captados, conversas e vendas originadas.',
          ...
          {season ? (
            <>
              <div className="grid grid-cols-3 gap-3">
                ...
                <Metric
                  label="Líder"
                  value={leader?.salesperson_name?.split(' ')[0] ?? '—'}
                  icon={Trophy}
                />
              </div>
            ...
            <p className="text-sm text-muted-foreground py-4">
              Nenhuma corrida ativa neste papel. Aguarde o admin iniciar a próxima
              temporada.
            </p>
          )}
  ...
  return (
    <RaceCalmProvider>
      <Helmet>
        ...
        <meta
          name="description"
          content="Hub central da Race Arena. Escolha entre a corrida dos Closers ou dos SDRs e acompanhe o ranking ao vivo."
        />
      </Helmet>
          ...
          {isAdmin && (
            <Button asChild variant="outline">
              <Link to="/admin/race-arena">
                <Settings className="w-4 h-4 mr-2" /> Admin Race
              </Link>
            </Button>
          )}
        </header>
              ...
              <CardContent className="p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-bold">
                    {career.summary.total_seasons} season
                    {career.summary.total_seasons === 1 ? '' : 's'}
                    {' · '}
                    <span className="text-warning">
                      {career.summary.total_titles} título
                      {career.summary.total_titles === 1 ? '' : 's'}
                    </span>
                    {' · '}
                    {career.summary.total_podiums} pódio
                    {career.summary.total_podiums === 1 ? '' : 's'}
                  </p>
                </div>
