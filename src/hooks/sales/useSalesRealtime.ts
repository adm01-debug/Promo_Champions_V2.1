...
export function useSalesRealtime(
  ...
  currentSalespersonRole?: SalespersonRole
) {
  ...
  const shouldReceiveNotification = useCallback(
    (sellerRole: SalespersonRole | null): boolean => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async payload => {
          ...
          if (!isWonSaleStatus(newSale.status) || !newSale.salesperson_id) return;
          ...
          toast.success(`🔥 ${salesperson.name} fechou uma venda!`, {
