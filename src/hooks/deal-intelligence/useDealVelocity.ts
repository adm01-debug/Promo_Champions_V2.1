...
import type {
  VelocityStatus,
  ConfidenceTier,
} from '@/components/deal-intelligence/velocityHelpers';
    ...
    const ch = supabase
      ...
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'deal_velocity_predictions',
...
export const usePredictDealVelocity = () => {
  ...
  return useMutation({
    mutationFn: async (input: { saleId?: string; batch?: boolean }) => {
      ...
      const { data, error } = await supabase.functions.invoke('predict-deal-velocity', {
        body,
      });
      if (error) throw error;
      return data;
