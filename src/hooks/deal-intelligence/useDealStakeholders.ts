...
import type {
  ...
  InfluenceLevel,
  Sentiment,
} from '@/components/deal-intelligence/committeeHelpers';
...
export const useUpsertStakeholder = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) => {
      ...
      supabase.functions
        .invoke('calculate-committee-coverage', { body: { sale_id: vars.sale_id } })
        .catch(() => {});
    },
...
export const useDeleteStakeholder = () => {
  ...
  return useMutation({
    ...
    onSuccess: ({ sale_id }) => {
      ...
      supabase.functions
        .invoke('calculate-committee-coverage', { body: { sale_id } })
        .catch(() => {});
...
export const useExtractStakeholders = () => {
  ...
  return useMutation({
    ...
    onSuccess: (
      data: { stakeholders?: Array<{ sale_id?: string }>; count?: number } | null
    ) => {
