...
export function useCompetencyData(salespersonId?: string) {
  ...
  return useQuery<CompetencyData[]>({
    ...
    queryFn: async () => {
      ...
      const prospectionActivities = activities.filter(
        a => a.activity_type === 'call' || a.activity_type === 'email'
      );
      const prospectionScore = Math.min(
        100,
        Math.round((prospectionActivities.length / totalActivities) * 150)
      );

      // Qualification: qualified leads ratio
      const qualifiedSales = sales.filter(s =>
        ['qualified', 'proposal', 'negotiation', 'completed'].includes(s.status)
      );
      const qualificationScore = Math.min(
        100,
        Math.round((qualifiedSales.length / totalSales) * 120)
      );
      ...
      const followUpScore = Math.min(
        100,
        Math.round((followUpActivities.length / totalActivities) * 160)
      );
      ...
      const presentationScore = Math.min(
        100,
        Math.round((successfulOutcomes.length / totalActivities) * 140)
      );
