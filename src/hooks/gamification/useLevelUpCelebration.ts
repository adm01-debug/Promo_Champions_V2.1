...
export function useLevelUpCelebration() {
  const { celebrateLevelUp, triggerLevelUpConfetti, sendPushNotification } =
    useCelebration();
  ...
  const triggerStreakMilestone = useCallback(
    async (event: StreakMilestoneEvent) => {
  ...
  const triggerNewRecordCelebration = useCallback(
    async (salespersonName: string, newRecord: number) => {
      const confetti = (await import('canvas-confetti')).default;
