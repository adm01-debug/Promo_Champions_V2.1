...
export function useGamifiedProfile(salespersonId?: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<GamifiedProfile | null> => {
      ...
      const [spRes, salesRes, streakRes, leagueRes, badgeRes, kudosRes, matchupRes] =
        await Promise.all([
          supabase
            .from('salespeople')
      ...
      (allSales || []).forEach(s => {
        salesBySp.set(
          s.salesperson_id ?? '',
          (salesBySp.get(s.salesperson_id ?? '') || 0) + Number(s.amount)
        );
      });
      ...
      const totalXp =
        badgeCount * 100 + (streakRes.data?.longest_streak || 0) * 50 + sales.length * 30;
