...
export function useV4CallbackActions() {
  ...
  const retryFiltered = useMutation({
    mutationFn: async ({ status, search }: { status: V4DeadLetterStatus; search: string }) => {
      let q = supabase.from("v4_callback_dead_letters")
        .update({ next_retry_at: new Date().toISOString(), attempts: 0, last_error: null })
        .select("id");
      if (status === "resolved") q = q.not("resolved_at", "is", null);
      else if (status === "pending") q = q.is("resolved_at", null).lt("attempts", MAX_ATTEMPTS);
      else if (status === "exhausted") q = q.is("resolved_at", null).gte("attempts", MAX_ATTEMPTS);
    ...
    onSuccess: (n) => { toast.success(`${n} evento(s) reprocessado(s) a partir do filtro`); invalidate(); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<V4Alert[]> => {
      ...
      const { data, error } = await (supabase as unknown as {
        from: (t: string) => { select: (c: string) => { gte: (c: string, v: string) => { order: (c: string, o: { ascending: boolean }) => { limit: (n: number) => Promise<{ data: V4Alert[] | null; error: Error | null }> } } } };
...
export function useV4AlertSettings() {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<V4AlertSettings | null> => {
      const { data, error } = await (supabase as unknown as {
        from: (t: string) => { select: (c: string) => { eq: (c: string, v: boolean) => { maybeSingle: () => Promise<{ data: V4AlertSettings | null; error: Error | null }> } } };
      }).from("v4_callback_alert_settings")
        .select("id, is_active, failure_rate_threshold, exhausted_threshold_24h, pending_threshold, window_minutes, min_events, suppress_minutes")
        .eq("singleton", true)
        .maybeSingle();
      if (error) throw error;
      return data;
  ...
  const mutation = useMutation({
    mutationFn: async (input: V4AlertSettingsInput) => {
      ...
      const { error } = await (supabase as unknown as {
        from: (t: string) => { update: (v: V4AlertSettingsInput) => { eq: (c: string, v: string) => Promise<{ error: Error | null }> } };
      }).from("v4_callback_alert_settings").update(input).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const ackMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await (supabase as unknown as {
        from: (t: string) => { update: (v: { acknowledged_at: string }) => { eq: (c: string, v: string) => Promise<{ error: Error | null }> } };
      }).from("v4_callback_alerts")
