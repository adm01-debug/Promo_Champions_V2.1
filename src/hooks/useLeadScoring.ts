...
export const useLeadScoring = (leadId?: string) => {
  return useQuery<ScoredLead[]>({
    ...
    queryFn: async (): Promise<ScoredLead[]> => {
      ...
      let serverScores: Record<
        string,
        { score: number; factors: ServerScoreFactors; labels: Record<string, string> }
      > = {};
      ...
      return clients
        .map(client => {
          ...
          const sourceScore =
            client.total_value > 100000 ? 10 : client.total_value > 50000 ? 7 : 5;
