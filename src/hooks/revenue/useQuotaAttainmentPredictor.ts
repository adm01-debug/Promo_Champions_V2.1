...
export function useQuotaForecasts() {
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'quota_attainment_forecasts' },
        () => {
...
export function useQuotaForecastSummary() {
  ...
  const avgProb =
    total > 0 ? forecasts.reduce((s, f) => s + f.attainment_probability, 0) / total : 0;
  return {
    total,
    safe,
    critical,
    ...
    criticalPct: total ? critical / total : 0,
  };
}
