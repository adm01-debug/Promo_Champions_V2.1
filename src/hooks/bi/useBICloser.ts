  ...
  return useQuery({
    ...
    queryFn: async (): Promise<BICloserData> => {
      ...
      const dailyRevenueNeeded =
        daysRemaining > 0 ? Math.max(0, (goal - totalRevenue) / daysRemaining) : 0;
      ...
      // Period comparisons
      const prevCompleted = previousSales.filter(s => isWonSaleStatus(s.status));
      ...
      const { data: allCloserSales } = await supabase
        ...
        .in(
          'salesperson_id',
          allClosers.map(c => c.id)
        );
      const closerRevenues: Record<string, number> = {};
      (allCloserSales || []).forEach(s => {
        if (s.salesperson_id)
          closerRevenues[s.salesperson_id] =
            (closerRevenues[s.salesperson_id] || 0) + Number(s.amount);
      });
      const rankings = Object.entries(closerRevenues)
        .sort((a, b) => b[1] - a[1])
        .map(([id], index) => ({ id, rank: index + 1 }));
      const currentRank =
        rankings.find(r => r.id === targetSalespersonId)?.rank || allClosers.length;

      // Client insights (extracted)
      const { topClients, highestTicketClients, clientPurchaseHistory } =
