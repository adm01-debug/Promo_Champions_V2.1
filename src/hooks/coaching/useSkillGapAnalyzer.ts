...
import type {
  ...
  SkillLevel,
  SkillTrend,
} from '@/components/coaching/skills/skillGapHelpers';
...
export const useSkillAssessments = (salespersonId?: string) =>
  useQuery({
    ...
    queryFn: async (): Promise<SkillAssessment[]> => {
      ...
      return (data ?? []).map(
        (r: { salespeople?: { name?: string } | null } & Record<string, unknown>) => ({
  ...
  useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('skill_assessments')
        .select('salesperson_id, skill, score, current_level, trend');
      if (error) throw error;
      const list = data ?? [];
      ...
      const skillAvgArr = Object.entries(skillAvg).map(([k, v]) => ({
        skill: k,
        avg: v.n ? v.sum / v.n : 0,
      }));
      const weakest = [...skillAvgArr].sort((a, b) => a.avg - b.avg)[0] ?? null;
      const strongest = [...skillAvgArr].sort((a, b) => b.avg - a.avg)[0] ?? null;
      const repsAtBeginner = new Set(
        list.filter(r => r.current_level === 'beginner').map(r => r.salesperson_id)
      ).size;
      return {
        total,
...
export const useAnalyzeSkillGaps = () => {
  ...
  return useMutation({
    ...
    onSuccess: data => {
      toast.success(
        `Análise concluída: ${data?.assessments ?? 0} avaliações, ${data?.tracks ?? 0} trilhas`
      );
