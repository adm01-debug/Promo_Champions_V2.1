...
export type CoachingSkillFocus =
  | 'discovery'
  | 'qualification'
  | 'objection_handling'
  | 'closing'
...
export const useCoachingOpportunities = (filters?: {
  severity?: CoachingSeverity;
  skillFocus?: CoachingSkillFocus;
}) =>
  useQuery({
...
export const useCoachingSummary = () =>
  useQuery({
    ...
    queryFn: async () => {
      ...
      const avgGap =
        list.length > 0
          ? list.reduce((s, r) => s + Number(r.gap_pct ?? 0), 0) / list.length
          : 0;
      return {
...
export const useDetectCoachingOpportunities = () => {
  ...
  return useMutation({
    ...
    onSuccess: data => {
      toast.success(
        `Análise concluída: ${data?.opportunities_detected ?? 0} oportunidades`
      );
