...
export function useReportMetrics(dateRange: DateRange) {
  ...
  const calculateMetrics = () => {
    ...
    const completedSales = salesQuery.data.filter(s => isWonSaleStatus(s.status));
    const totalRevenue = completedSales.reduce((sum, s) => sum + Number(s.amount), 0);
    const activationRevenue = completedSales
    ...
    if (metricsQuery.data && metricsQuery.data.length >= 2) {
      ...
      const firstTicket =
        firstHalf.length > 0
          ? firstHalf.reduce((sum, m) => sum + Number(m.avg_ticket), 0) / firstHalf.length
          : 0;
      const secondTicket =
        secondHalf.length > 0
