...
export const MentionInput = React.memo(
  ({ value, onChange, onSubmit, placeholder, disabled }: MentionInputProps) => {
      ...
      {parts.map((part, i) =>
        part.startsWith('@') ? (
          <Badge
            key={i}
            variant="secondary"
            className="text-[10px] px-1 py-0 mx-0.5 bg-primary/10 text-primary border-primary/20"
          >
            {part}
          </Badge>
        ) : (
