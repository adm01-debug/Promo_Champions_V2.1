...
export const GoalTracker: FC<GoalTrackerProps> = ({
  ...
  return (
    <Card className="p-5 glass border-border/40 hover:border-primary/40 transition-all duration-300 hover-lift relative overflow-hidden group">
      ...
      <div
        className={`absolute -right-8 -top-8 w-24 h-24 rounded-full blur-3xl opacity-10 transition-opacity group-hover:opacity-20 ${status.color}`}
      />

      <div className="flex items-start justify-between mb-6 relative z-10">
        <div className="flex items-center gap-4">
          <div
            className={`p-3 rounded-2xl ${status.color}/10 shadow-inner border border-border/10`}
          >
        ...
        <Badge
          className={`${status.color} text-primary-foreground font-black uppercase tracking-widest text-[9px] px-2.5 py-1 border-none shadow-lg`}
        >
        ...
        <div className="flex justify-between items-end mb-2.5 px-0.5">
          <div className="flex flex-col">
            ...
            <span className="text-lg font-black tracking-tighter gradient-text">
              ...
              <span className="text-[10px] text-muted-foreground font-bold italic ml-1">
                {goal.unit}
              </span>
            </span>
          </div>
          ...
          <div
            className={`absolute h-full transition-all duration-1000 ease-out flex items-center justify-end px-2 ${
              goal.status === 'completed'
                ? 'bg-gradient-to-r from-status-info via-primary to-status-info shadow-[0_0_15px_rgba(var(--primary),0.4)]'
                : status.color === 'bg-status-success'
                  ...
                  : status.color === 'bg-status-warning'
                    ? 'bg-gradient-to-r from-status-warning to-status-warning/60 shadow-[0_0_12px_rgba(245,158,11,0.3)]'
      ...
      {showDetails && (
        <div className="flex items-center justify-between text-sm pt-4 border-t border-border/10 relative z-10">
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <div className="flex flex-col">
              <span className="text-[9px] uppercase font-black tracking-widest opacity-60 leading-none">
                Vencimento
              </span>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-section-title">Minhas Metas</h3>
    ...
    <Card className="p-6 glass border-border/40 card-elevated overflow-hidden relative group">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="font-display font-black text-xl tracking-tighter uppercase italic gradient-text">
              Roadmap de Elite
            </h4>
        ...
        <div className="flex flex-col items-end">
          <Badge
            variant="outline"
            className="text-[10px] font-black uppercase tracking-widest px-3 py-1 border-primary/20 text-primary mb-1 bg-primary/5"
          >
      ...
      <div className="space-y-4 relative z-10">
        {milestones.map((milestone, _index) => (
          ...
          >
            <div
              className={`absolute inset-0 bg-gradient-to-r from-status-success/5 to-transparent transition-opacity duration-700 ${milestone.isCompleted ? 'opacity-100' : 'opacity-0'}`}
            />
            ...
            >
              {milestone.isCompleted ? (
                ...
                <div className="w-2 h-2 rounded-full bg-muted-foreground/20 group-hover/milestone:bg-primary/40 transition-colors" />
              )}
            </button>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2 mb-1">
                <p
                  className={`font-display font-bold text-sm tracking-tight ${milestone.isCompleted ? 'text-muted-foreground/70' : 'text-foreground'}`}
                >
                  {milestone.title}
                </p>
                {milestone.isCompleted && (
              ...
              <p
                className={`text-xs leading-relaxed ${milestone.isCompleted ? 'text-muted-foreground/50' : 'text-muted-foreground'}`}
              >
              ...
              <div className="flex items-center gap-3 mt-3">
                <div className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest">
                  <Clock
                    className={`h-3 w-3 ${milestone.isCompleted ? 'text-muted-foreground/40' : 'text-primary'}`}
                  />
                  <span
                    className={
                      milestone.isCompleted
                  ...
                  >
                    {milestone.isCompleted
                      ? `Finalizado: ${milestone.completedAt}`
