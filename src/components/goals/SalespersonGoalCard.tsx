...
function SalespersonGoalCardImpl({
  name,
  avatar_url,
  role,
  goalAmount,
  ...
  return (
    <div
      className={`group p-5 rounded-2xl border transition-all duration-500 hover:scale-[1.02] cursor-pointer relative overflow-hidden ${
        hasExceededGoal
    ...
    >
      ...
      <div
        className={`absolute -top-24 -right-24 w-48 h-48 blur-[80px] opacity-20 pointer-events-none transition-opacity group-hover:opacity-40 ${
          hasExceededGoal
            ? 'bg-status-success'
            : onTrack
              ? 'bg-primary'
      ...
      <div className="flex items-start gap-4 relative z-10">
        ...
        <div className="relative">
          <div
            className={`absolute -inset-1 rounded-full blur-sm opacity-0 group-hover:opacity-60 transition-opacity ${
              hasExceededGoal ? 'bg-status-success' : 'bg-primary'
            }`}
          />
          <Avatar
            className={`h-14 w-14 shadow-2xl relative border-2 transition-transform duration-500 group-hover:scale-110 ${
          ...
          {rank <= 3 && (
            <div
              className={`absolute -top-1 -left-1 h-7 w-7 rounded-full flex items-center justify-center shadow-lg border-2 border-background z-20 ${
                rank === 1
                  ? 'bg-rank-gold'
                  : rank === 2
                    ? 'bg-rank-silver'
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Badge
              variant="outline"
              className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 bg-background/50 border-border/40"
            >
        ...
        <div className="text-right">
          <p
            className={`text-xl font-display font-bold ${hasExceededGoal ? 'text-status-success' : 'gradient-text'}`}
          >
            {progress.toFixed(0)}%
          </p>
      ...
      {goalAmount > 0 && (
        <div className="mt-6 grid grid-cols-2 gap-3 relative z-10">
          ...
          <div
            className={`p-3 rounded-2xl border flex flex-col items-center justify-center group/stat transition-all ${
          ...
          >
            <div className="flex items-center gap-1.5 mb-1">
              <Target
                className={`h-3 w-3 group-hover/stat:scale-110 transition-transform ${
                  requiredDailyAverage > dailyAverage
                    ? 'text-status-warning'
                    : 'text-status-success'
                }`}
      ...
      <div className="absolute top-4 right-4 text-right flex flex-col items-end">
        <span
          className={`text-2xl font-display font-black tracking-tighter italic leading-none ${
            hasExceededGoal
              ? 'text-status-success drop-shadow-glow-sm'
              : 'text-foreground/80'
          }`}
        >
          {progress.toFixed(0)}%
        </span>
        <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground/40 mt-1">
          Rank #{rank}
        </span>
      </div>
    </div>
...
export const SalespersonGoalCard = React.memo(SalespersonGoalCardImpl);
