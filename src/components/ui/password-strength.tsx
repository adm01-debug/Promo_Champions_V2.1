...
export function PasswordStrength({ password, className, onStrengthChange }: PasswordStrengthProps) {
  const [leakCheck, setLeakCheck] = useState<{ checking: boolean; leaked: boolean; count: number }>({
    checking: false, leaked: false, count: 0
  });
  ...
  const strength = useMemo(() => {
    ...
    if (leakCheck.leaked) return { score: 10, label: "Comprometida!", color: "bg-destructive" };
    if (metCount <= 1) return { score: 20, label: "Muito fraca", color: "bg-destructive" };
    if (metCount === 2) return { score: 40, label: "Fraca", color: "bg-streak" };
    if (metCount === 3) return { score: 60, label: "Média", color: "bg-warning" };
    if (metCount === 4) return { score: 80, label: "Forte", color: "bg-success" };
    return { score: 100, label: "Muito forte", color: "bg-success" };
  ...
  const getIcon = () => {
    if (leakCheck.leaked) return <AlertTriangle className="h-5 w-5 text-destructive animate-pulse" />;
    if (strength.score <= 40) return <ShieldAlert className="h-5 w-5 text-destructive" />;
    if (strength.score <= 60) return <Shield className="h-5 w-5 text-warning" />;
    ...
    <div className={cn("space-y-3", className)}>
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {leakCheck.checking && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />}
            <span className={cn(
              "text-sm font-semibold",
              leakCheck.leaked && "text-destructive",
              !leakCheck.leaked && strength.score <= 40 && "text-destructive",
              !leakCheck.leaked && strength.score > 40 && strength.score <= 60 && "text-warning",
              !leakCheck.leaked && strength.score > 60 && "text-success"
            )}>
              {strength.label}
            </span>
          </div>
        ...
        <div className="h-2 bg-muted rounded-full overflow-hidden">
          <div className={cn("h-full transition-all duration-500 ease-out rounded-full", strength.color)} style={{ width: `${strength.score}%` }} />
        </div>
      </div>
      ...
      {leakCheck.leaked && (
        <div className="flex items-start gap-2 p-3 rounded-lg bg-destructive/10 border border-destructive/30 animate-fade-in">
          ...
          <div className="space-y-1">
            <p className="text-sm font-semibold text-destructive">Senha comprometida!</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {requirements.map((req, index) => (
          <div key={index} className={cn(
            ...
            req.met ? "bg-success/10 text-success" : "bg-muted/50 text-muted-foreground"
          )}>
        ...
        <div className={cn(
          ...
          !leakCheck.checking && !leakCheck.leaked && password.length >= 8 && "bg-success/10 text-success",
          leakCheck.leaked && "bg-destructive/10 text-destructive"
        )}>
          {leakCheck.checking ? (
            <><Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin" /><span>Verificando vazamentos...</span></>
          ) : leakCheck.leaked ? (
            <><AlertTriangle className="h-3.5 w-3.5 shrink-0" /><span>Senha encontrada em vazamentos</span></>
          ) : password.length >= 8 ? (
            <><ShieldCheck className="h-3.5 w-3.5 shrink-0" /><span>Sem vazamentos conhecidos</span></>
          ) : (
            <><Shield className="h-3.5 w-3.5 shrink-0" /><span>Verificação após 8 caracteres</span></>
          )}
        </div>
      </div>
    </div>
