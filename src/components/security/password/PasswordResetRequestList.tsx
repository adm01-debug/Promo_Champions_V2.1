    ...
    <ScrollArea className="h-[500px]">
      <div className="space-y-3">
        {filteredRequests.map(request => (
          <div
            key={request.id}
            className={`p-4 rounded-lg border transition-colors ${request.status === 'pending' && !isExpired(request.expires_at) ? 'bg-rank-gold/5 border-rank-gold/20' : 'bg-muted/30 border-border'}`}
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-muted-foreground">
                  ...
                  <div>
                    ...
                    <p
                      className={`font-medium ${isExpired(request.expires_at) ? 'text-destructive' : 'text-foreground'}`}
                    >
              ...
              {request.status === 'pending' && !isExpired(request.expires_at) && (
                <div className="flex gap-2 ml-4">
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Um email de reset de senha será enviado para{' '}
                  ...
                  <Dialog
                    open={rejectDialogOpen && selectedRequest?.id === request.id}
                    onOpenChange={open => {
