...
import { Crown, Zap, Loader2, Eye, EyeOff, ArrowRight, Lock } from 'lucide-react';
...
interface AuthFormCardProps {
  ...
  handleMfaVerify: (e: React.FormEvent) => void;
}
        ...
        <div className="relative rounded-2xl bg-[#0a0b1a]/95 backdrop-blur-xl p-7 sm:p-9 border border-white/5">
          <div className="mb-7">
            <div className="flex items-center gap-2 mb-2">
              ...
              />
              <span className="text-[10px] tracking-[0.3em] text-cyan-300 font-bold">
                ACESSO AO CIRCUITO
              </span>
            </div>
          ...
          <div className="mb-6 p-3 rounded-xl bg-cyan-500/5 border border-cyan-400/20 flex items-start gap-2.5">
            <Lock
              className="h-4 w-4 text-cyan-400 mt-0.5 flex-shrink-0"
              style={{ filter: 'drop-shadow(0 0 6px #22d3ee)' }}
            />
            <p className="text-[11px] text-white/60 leading-relaxed">
              Acesso restrito à equipe Promo Brindes. Novos cadastros são criados
          ...
          {mfaRequired ? (
            <form onSubmit={handleMfaVerify} className="space-y-4">
              <div className="space-y-1.5">
                <Label className="text-[11px] font-bold text-white/40 ml-1 uppercase tracking-wider">
                ...
                <Input
                  ...
                  autoFocus
                  className="bg-white/5 border-white/10 focus:border-cyan-400/50 focus:ring-cyan-400/20 h-11 transition-all rounded-xl text-center text-xl tracking-[0.4em] font-mono"
                  required
                />
              </div>
              <Button
                type="submit"
                disabled={isMfaLoading || mfaCode.replace(/\s/g, '').length < 6}
                className="relative w-full h-12 mt-4 bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-white font-black uppercase tracking-[0.2em] rounded-xl overflow-hidden shadow-[0_0_20px_rgba(34,211,238,0.4)]"
              >
                {isMfaLoading ? (
            ...
            <>
              ...
              <div className="relative my-6">
                ...
                <div className="relative flex justify-center text-[10px] uppercase tracking-widest font-bold">
                  <span className="bg-[#0a0b1a] px-3 text-white/60">
                    ou via credenciais
                  </span>
                </div>
              </div>
              ...
              <form onSubmit={handleAuth} className="space-y-4">
                <div className="space-y-1.5">
                  <Label
                    htmlFor="email"
                    className="text-[11px] font-bold text-white/40 ml-1 uppercase tracking-wider"
                  >
                    E-mail de Acesso
                  </Label>
                  <Input
                      ...
                      <DialogContent className="bg-[#0a0b1a] border-white/10 text-white">
                        ...
                        <form onSubmit={handlePasswordReset} className="space-y-4 pt-4">
                          ...
                          <Button
                            ...
                            className="w-full bg-cyan-500 hover:bg-cyan-400 text-[#05060f] font-black uppercase tracking-widest"
                          >
                            {isResetLoading ? (
                              <Loader2 className="h-4 w-4 animate-spin" />
                            ) : (
