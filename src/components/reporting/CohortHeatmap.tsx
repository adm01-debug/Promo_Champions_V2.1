...
import {
  useCohortRetention,
  type CohortMetric,
} from '@/hooks/reporting/useCohortRetention';
...
export const CohortHeatmap = memo(
  ({ embedded = false, defaultPeriods = 12, defaultMetric = 'orders' }: Props) => {
        ...
        {!embedded && (
          <CardHeader className="pb-3">
            ...
            <p className="text-xs text-muted-foreground">
              % de clientes ativos por mês relativo à aquisição. Cores mais quentes =
              maior retenção.
            </p>
          </CardHeader>
        )}
                  ...
                  <thead className="bg-muted/30">
                    <tr>
                      ...
                      {periodLabels.map(l => (
                        <th
                          key={l}
                          className="text-center px-1.5 py-2 font-medium text-muted-foreground min-w-[44px]"
                        >
                          {l}
                        </th>
                      ))}
...
function KpiCard({ icon: Icon, label, value, accent }: KpiCardProps) {
  return (
    <div
      className={`rounded-lg border p-3 transition-colors ${accent ? 'border-primary/40 bg-primary/5' : 'border-border/40 bg-card/50'}`}
    >
