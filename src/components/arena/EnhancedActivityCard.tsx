          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Badge
                  variant="outline"
                  className="text-[8px] font-black uppercase tracking-widest bg-white/5 h-4"
                >
                  {data.role}
                </Badge>
                {isHighPerformer && (
              ...
              <div className="flex flex-col gap-1.5 mt-3 group/xp">
                <div className="flex justify-between items-center">
                  <span className="text-[8px] font-black text-primary uppercase tracking-[0.2em] animate-pulse">
                    Nível {Math.floor(data.progress.overall / 10) + 1}
                  </span>
                  <span className="text-[8px] font-black text-muted-foreground uppercase tracking-widest">
                    {((data.progress.overall % 10) * 10).toFixed(0)}/100 XP
                  </span>
                </div>
          ...
          <div className="text-right flex flex-col items-end">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {reactions > 0 && (
                <Badge
                  variant="secondary"
                  className="h-4 px-1.5 text-[8px] bg-primary/10 text-primary border-none font-black animate-in fade-in zoom-in"
                >
          ...
          <div className="relative h-2 w-full bg-white/5 rounded-full overflow-hidden">
            <div
              className={cn(
                ...
                isWinner
                  ? 'bg-gradient-to-r from-rank-gold via-yellow-400 to-rank-gold animate-shimmer'
                  : 'bg-primary'
              )}
            ...
            {[
              {
                icon: Target,
                label: 'Calls',
                val: data.current.calls,
                goal: data.goals.calls,
                color: 'text-blue-400',
              },
              {
                icon: Zap,
                label: 'Lead Gen',
                val: data.current.emails,
                goal: data.goals.emails,
                color: 'text-purple-400',
              },
              {
                icon: TrendingUp,
                label: 'Deals',
                val: data.current.meetings,
                goal: data.goals.meetings,
                color: 'text-emerald-400',
              },
            ].map((stat, i) => (
              <div
                key={i}
                className="bg-white/5 rounded-xl p-3 border border-white/5 transition-colors group-hover:bg-white/10"
              >
