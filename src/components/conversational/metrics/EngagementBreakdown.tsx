...
export function EngagementBreakdown({ metrics }: Props) {
  ...
  const rows = [
    {
      label: 'Balanço de fala',
      value: balance,
      hint: `Vendedor ${formatPct(metrics.seller_talk_ratio)}`,
    },
    {
      label: 'Baixo silêncio',
      value: silenceOk,
      hint: `Silêncio ${formatPct(metrics.silence_ratio)}`,
    },
    {
      label: 'Sem interrupções',
      value: interruptOk,
      hint: `${metrics.interruptions_count} interrupções`,
    },
    {
      label: 'Monólogos curtos',
      value: monologueOk,
      hint: `Maior ${metrics.longest_monologue_seconds}s`,
    },
  ];
