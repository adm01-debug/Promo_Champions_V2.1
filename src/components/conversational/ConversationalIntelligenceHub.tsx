...
import {
  ...
  useRecordingSummaryDrawer,
} from './RecordingSummaryDrawer';
...
export const ConversationalIntelligenceHub = () => {
  ...
  const { data, isLoading } = useConversationalIntelligence(
    horizon === 'live' ? 0 : horizon
  );
  ...
  return (
    <div className="space-y-12 relative pb-20">
      ...
      <div className="fixed inset-0 -z-10 pointer-events-none">
        ...
        <div
          className="absolute bottom-[10%] right-[5%] w-[500px] h-[500px] bg-info/5 blur-[100px] rounded-full animate-pulse"
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8 border-b border-white/5 pb-10">
        <div className="space-y-3">
          ...
          <h1 className="text-5xl font-black font-sora tracking-tight bg-gradient-to-br from-foreground via-foreground to-foreground/40 bg-clip-text text-transparent sm:text-6xl">
            ...
            <Badge className="bg-primary/10 text-primary border-primary/20 text-[10px] font-black uppercase ml-2 align-middle">
              10/10
            </Badge>
          </h1>
          <p className="text-base text-muted-foreground/80 max-w-2xl font-medium leading-relaxed">
            Decifre cada palavra, tom e hesitação. Análise neural de voz com transcrição,
            diarização e coaching em tempo real integrada aos principais players do
            mercado.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 bg-white/5 p-2 rounded-2xl border border-white/10 backdrop-blur-xl shadow-2xl">
          ...
          <Tabs
            ...
            onValueChange={v => setHorizon(v === 'live' ? 'live' : Number(v))}
            className="bg-transparent"
          >
            <TabsList className="bg-white/5 border-none p-1">
              <TabsTrigger value="7" className="text-xs font-black px-4">
                7D
              </TabsTrigger>
              <TabsTrigger value="30" className="text-xs font-black px-4">
                30D
              </TabsTrigger>
              <TabsTrigger value="90" className="text-xs font-black px-4">
                90D
              </TabsTrigger>
              <TabsTrigger
                value="live"
                className="text-xs font-black px-5 bg-destructive/10 text-destructive data-[state=active]:bg-destructive data-[state=active]:text-white transition-all gap-2"
              >
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            <KPICard
              icon={Mic}
              label="Mapeamento de Voz"
              value={data.kpis.total_calls}
              hint={`${data.kpis.coverage_percent}% coverage`}
              trend="+12%"
              accent="text-primary"
            />
            <KPICard
              ...
              value={`${data.kpis.total_duration_minutes}m`}
              hint={`Média ${data.kpis.avg_duration_minutes}m / call`}
              trend="-5%"
              accent="text-info"
            />
            <KPICard
              ...
              value={data.kpis.avg_sentiment.toFixed(2)}
              hint="Escala de Empatia (-1 a +1)"
              trend="+0.05"
              accent="text-success"
            />
            <KPICard
              icon={MessageSquare}
              label="Ratio de Persuasão"
              value={`${Math.round(data.kpis.avg_talk_ratio_salesperson * 100)}%`}
              hint={`${data.kpis.avg_questions_per_call} Q/call`}
              trend="Ideal"
              accent="text-warning"
            />
          </div>
              ...
              <CardContent className="space-y-4 pt-6">
                {data.top_objections.length === 0 ? (
                  ...
                  data.top_objections.map(o => (
                    <div
                      key={o.label}
                      className="flex justify-between items-center p-3 rounded-xl border border-white/5 hover:bg-white/5 transition-all group/item"
                    >
                      <span className="text-xs font-bold group-hover/item:text-warning transition-colors">
                        {o.label}
                      </span>
                      <Badge className="bg-warning/20 text-warning border-none font-black text-[10px]">
                        {o.count} ocorrências
                      </Badge>
                    </div>
                  ))
                ...
                {data.top_topics.length === 0 ? (
                  ...
                  data.top_topics.map(t => (
                    <div
                      ...
                      className="flex justify-between items-center p-3 rounded-xl border border-white/5 hover:bg-white/5 transition-all group/item"
                    >
                      <span className="text-xs font-bold group-hover/item:text-primary transition-colors">
                        {t.label}
                      </span>
                      <Badge
                        variant="outline"
                        className="border-primary/20 text-primary font-black text-[10px]"
                      >
                        {t.count} citações
                      </Badge>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2">
                <Card className="h-[650px] flex flex-col items-center justify-center border border-primary/20 bg-slate-950/60 relative overflow-hidden group shadow-[0_0_50px_rgba(var(--primary-rgb),0.1)]">
                  ...
                  <div className="relative z-10 flex flex-col items-center text-center px-10">
                    ...
                    <div className="mt-8 space-y-4">
                      <div className="flex items-center justify-center gap-3">
                        <Badge className="bg-primary text-primary-foreground border-none animate-pulse font-black text-xs px-5 py-1.5 shadow-[0_0_15px_rgba(var(--primary-rgb),0.3)]">
                          NEURAL LINK ACTIVE
                        </Badge>
                        <span className="text-xs font-black text-primary tabular-nums tracking-widest bg-primary/10 px-3 py-1 rounded-md border border-primary/20">
                          00:12:45
                        </span>
                      </div>

                      <h3 className="text-4xl font-black font-sora tracking-tighter uppercase italic text-white">
                        Neural <span className="text-primary">Coaching</span> Copilot
                      </h3>
                      <p className="text-sm text-muted-foreground max-w-md leading-relaxed font-medium">
                        Escaneando ondas sonoras via Twilio Bridge. Nossa rede neural está
                        processando intenções e injetando battlecards estratégicos em
                        tempo real.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-16 w-full max-w-3xl">
                      ...
                      >
                        ...
                        <p className="text-[10px] font-black uppercase text-primary mb-2 tracking-widest flex items-center gap-2">
                          <Sparkles className="size-3 animate-pulse" /> IA Sugestão:
                          Próxima Resposta
                        </p>
                        <p className="text-sm font-bold italic text-white leading-relaxed">
                          "Entendo a preocupação com o CAPEX. Podemos estruturar o
                          contrato como OPEX via nossa parceria de leasing para facilitar
                          a aprovação imediata."
                        </p>
                        <div className="mt-3 flex gap-2">
                          <Badge className="bg-primary/20 text-primary border-primary/30 text-[9px] uppercase font-black">
                            Contexto: CAPEX/Budget
                          </Badge>
                          <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[9px] uppercase font-black">
                            Prob. Sucesso: 88%
                          </Badge>
                        </div>
                      </motion.div>
                      ...
                      >
                        ...
                        <p className="text-xs font-medium text-white/90 leading-relaxed mb-3">
                          Destaque que nossa implementação leva 15 dias vs 6 meses do
                          concorrente mencionado. O custo de oportunidade da demora supera
                          o desconto oferecido por eles.
                        </p>
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-6 text-[9px] font-black uppercase tracking-widest border-warning/20 text-warning hover:bg-warning/10"
                        >
                          Ver Talking Points
                        </Button>
                      </motion.div>
                  ...
                  <div
                    className="absolute top-20 right-20 h-1.5 w-1.5 bg-primary rounded-full animate-ping"
          ...
          <Card className="glass border-white/5 shadow-2xl overflow-hidden">
            <CardHeader className="border-b border-white/5 bg-white/5 pb-8">
              ...
              <CardDescription>
                Busca semântica nos transcripts — encontre objeções, concorrentes e
                tópicos
              </CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {r.sentiment_label && (
                              <Badge
                                variant="outline"
                                className={sentimentColor(r.sentiment_label)}
                              >
                                {r.sentiment_label}
                              </Badge>
                            )}
                          ...
                          <div className="flex gap-4 mt-2 text-xs text-muted-foreground flex-wrap">
                            <span>
                              {format(new Date(r.recorded_at), "dd MMM yyyy 'às' HH:mm", {
                                locale: ptBR,
                              })}
                            </span>
                            <span>{Math.round(r.duration_seconds / 60)}min</span>
                            {r.questions_asked != null && (
                              <span>{r.questions_asked} perguntas</span>
                            )}
                            {r.objections_count > 0 && (
                              <span className="text-warning">
                                {r.objections_count} objeções
                              </span>
                            )}
                            {r.next_steps_count > 0 && (
                              <span className="text-success">
                                {r.next_steps_count} próximos passos
                              </span>
                            )}
                          </div>
                        </div>
                        {r.talk_ratio_salesperson != null && (
                          <div className="text-right">
                            ...
                            <p className="text-sm font-semibold">
                              {Math.round(r.talk_ratio_salesperson * 100)}% /{' '}
                              {Math.round((r.talk_ratio_client ?? 0) * 100)}%
                            </p>
                          </div>
                        )}
                      </div>
...
const KPICard = ({
  ...
  <Card className="glass border-white/5 hover:border-primary/30 transition-all duration-500 group relative overflow-hidden">
    <div
      className={cn(
        'absolute -right-4 -top-4 w-20 h-20 blur-2xl opacity-0 group-hover:opacity-10 transition-opacity',
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            <p
              className={cn(
                'text-4xl font-black font-display tracking-tighter drop-shadow-sm',
                accent
              )}
            >
              {value}
        ...
        <div
          className={cn(
            'size-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center transition-all duration-500 shadow-xl group-hover:scale-110 group-hover:bg-white/10',
            accent
          )}
        >
