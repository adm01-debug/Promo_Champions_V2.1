        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            {(['A', 'B', 'C'] as const).map((cls, index) => (
              <SummaryCard
                key={`prod-${cls}`}
                cls={cls}
                icon={<Package className="h-3.5 w-3.5" style={{ color: COLORS[cls] }} />}
                label="Produtos"
                value={data?.summary.products[cls] || 0}
                delay={index * 50}
              />
            ))}
            {(['A', 'B', 'C'] as const).map((cls, index) => (
              <SummaryCard
                key={`client-${cls}`}
                cls={cls}
                icon={<Users className="h-3.5 w-3.5" style={{ color: COLORS[cls] }} />}
                label="Clientes"
                value={data?.summary.clients[cls] || 0}
                delay={(index + 3) * 50}
              />
            ))}
          </div>
          ...
          <Tabs defaultValue="products" className="space-y-4">
            <TabsList className="glass border border-border/50">
              <TabsTrigger
                value="products"
                className="gap-2 font-display transition-all duration-300 data-[state=active]:bg-gradient-to-r data-[state=active]:from-primary data-[state=active]:to-accent data-[state=active]:text-primary-foreground data-[state=active]:shadow-md"
              >
            ...
            <TabsContent value="products" className="animate-fade-in">
              <ABCChartTable
                items={data?.products || []}
                chartTitle="Curva ABC - Produtos"
                tableTitle="Ranking de Produtos"
                chartIcon={null}
                tableIcon={<Package className="h-4 w-4 text-primary-foreground" />}
                emptyIcon={<Package className="h-10 w-10 opacity-50" />}
              />
            </TabsContent>
            ...
            <TabsContent value="clients" className="animate-fade-in">
              <ABCChartTable
                items={data?.clients || []}
                chartTitle="Curva ABC - Clientes"
                tableTitle="Ranking de Clientes"
                chartIcon={null}
                tableIcon={<Users className="h-4 w-4 text-primary-foreground" />}
                emptyIcon={<Users className="h-10 w-10 opacity-50" />}
              />
            </TabsContent>
          </Tabs>
        </div>
            ...
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground leading-tight">
                  Elite engine detectou 3 novas oportunidades de automação.
                </span>
              </div>
            </CardContent>
          </Card>
  ...
  return (
    ...
    >
      <CardContent className="p-4">
        ...
        <p
          className="text-2xl font-bold font-display transition-transform duration-300 group-hover:scale-105"
