...
function computeHealthScore(client: {
  ...
  if (client.total_value > 50000) {
    score += 20;
    factors.push('High Asset Value');
  } else if (client.total_value > 10000) {
    score += 10;
    factors.push('Moderate Revenue');
  } else {
    score -= 10;
    factors.push('Low Revenue Profile');
  }

  if (client.last_purchase_date) {
    const daysSince = Math.floor(
      (Date.now() - new Date(client.last_purchase_date).getTime()) / 86400000
    );
    if (daysSince < 30) {
      score += 20;
      factors.push('Recent Signal Detected');
    } else if (daysSince < 90) {
      score += 5;
      factors.push('Active within 90d window');
    } else {
      score -= 15;
      factors.push(`Signal Lost: ${daysSince} days`);
    }
  } else {
    score -= 20;
  ...
  if (client.activity_count >= 5) {
    score += 10;
    factors.push('High Engagement Telemetry');
  } else if (client.activity_count >= 2) {
    score += 5;
  } else {
    score -= 10;
    factors.push('Engagement Anomalies Detected');
  }
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/5 bg-black/40 backdrop-blur-xl p-5 space-y-6 group">
      ...
      <div className="flex items-center gap-3 relative z-10">
        ...
        <div>
          <h3 className="text-xs font-mono font-bold uppercase tracking-[0.3em] text-primary">
            Biometric Asset Status
          </h3>
        ...
        {(['healthy', 'at_risk', 'critical'] as const).map(cat => {
          ...
          return (
            <div
              key={cat}
              className={cn(
                'relative rounded-xl p-3 border text-center group/item transition-all hover:bg-white/5',
                config.color
              )}
            >
              <Icon className="h-3.5 w-3.5 mx-auto mb-1.5" />
              <p className="text-2xl font-mono font-black tabular-nums">{summary[cat]}</p>
              <p className="text-[8px] font-mono font-bold uppercase tracking-widest opacity-60">
                {config.label}
              </p>
                ...
                return (
                  ...
                  >
                    <div className="flex-1 min-w-0 pr-4">
                      <p className="text-[11px] font-mono font-black uppercase truncate group-row:text-primary transition-colors">
                        {h.client_name}
                      </p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <Activity className="h-2.5 w-2.5 text-muted-foreground/40" />
                        <p className="text-[8px] font-mono text-muted-foreground/60 uppercase tracking-tighter truncate">
                          {h.factors[0]}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="text-right">
                        <span
                          className={cn(
                            ...
                            h.score < 40 ? 'text-destructive' : 'text-warning'
                          )}
                        >
                          {h.score}
                        </span>
                        <p className="text-[7px] font-mono uppercase tracking-tighter text-muted-foreground/40">
                          Score
                        </p>
                      </div>
                      <Badge
                        variant="outline"
                        className={cn(
                          'text-[8px] font-mono font-black uppercase tracking-widest h-5 px-1.5 bg-black/40',
                          config.color
                        )}
                      >
                        {config.label}
