  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [salesRes, pipelineRes, goalsRes, historyRes] = await Promise.all([
        supabase
          .from('sales')
          .select('id, amount, status, created_at, salesperson_id')
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <MetricCard
          icon={DollarSign}
          label="Receita Atual"
          value={fmt(forecast.currentRevenue)}
          color="text-success"
        />
        <MetricCard
          icon={TrendingUp}
          label="Projeção Mês"
          value={fmt(forecast.combinedForecast)}
          color="text-primary"
          subtitle={`${forecast.confidence}% confiança`}
        />
        <MetricCard
          icon={Target}
          label="Meta Total"
          value={fmt(forecast.totalGoal)}
          color="text-accent"
          subtitle={`${forecast.goalProgress.toFixed(1)}% atingido`}
        />
        <MetricCard
            ...
            >
              {forecast.willHitGoal ? (
                <span className="flex items-center justify-center gap-1">
                  <ArrowUpRight className="h-4 w-4" /> Projeção acima da meta em{' '}
                  {fmt(Math.abs(forecast.gapToGoal))}
                </span>
              ) : (
                <span className="flex items-center justify-center gap-1">
