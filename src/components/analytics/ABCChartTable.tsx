...
function ABCTooltip({ active, payload }: RechartsTooltipProps) {
  ...
  return (
    <div className="glass rounded-xl p-4 border border-border/50 shadow-xl animate-fade-in">
      <p className="font-display font-semibold text-foreground gradient-text">
        {data.name}
      </p>
      ...
      <Badge
        ...
        style={{
          borderColor: COLORS[data.classification],
          color: COLORS[data.classification],
        }}
      >
...
export const ABCChartTable = React.memo(function ABCChartTable({
  items,
  chartTitle,
  tableTitle,
  ...
  const handleBulkAction = (action: string) => {
    toast.info(
      `${action} aplicado a ${selectedItems.length} itens: ${selectedItems.join(', ')}`
    );
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      size="sm"
                      variant="outline"
                      className="glass gap-2 border-primary/30 hover:border-primary/60 transition-all duration-300"
                    >
                  ...
                  <DropdownMenuContent
                    align="end"
                    className="glass border-border/50 min-w-[180px]"
                  >
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
                      onClick={() => handleBulkAction('Adicionar Tag')}
                    ...
                    <DropdownMenuItem
                      onClick={() => handleBulkAction('Mover p/ Campanha')}
                      className="gap-2 cursor-pointer"
                    >
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 glass z-10">
                  <tr className="border-b border-border/50">
                    ...
                    <th className="text-left py-3 px-3 text-muted-foreground font-display font-medium">
                      Nome
                    </th>
                    <th className="text-right py-3 px-3 text-muted-foreground font-display font-medium">
                      Receita
                    </th>
                    <th className="text-right py-3 px-3 text-muted-foreground font-display font-medium">
                      %
                    </th>
                    <th className="text-center py-3 px-3 text-muted-foreground font-display font-medium">
                      Classe
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((item, i) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 font-medium text-foreground transition-colors group-hover:text-primary">
                        {item.name}
                      </td>
