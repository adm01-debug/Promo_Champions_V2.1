        ...
        {statCards.map((card, i) => {
          ...
          return (
            <Card
              key={card.label}
              variant="elevated"
              className={`glass ${card.border} dark:border-glow hover-lift transition-all group animate-fade-in`}
              style={{ animationDelay: `${i * 50}ms` }}
            >
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2.5 rounded-xl ${card.gradient ? `bg-gradient-to-br ${card.gradient}` : card.iconBg} shadow-md transition-all duration-300 group-hover:scale-110`}
                  >
                    <Icon
                      className={`h-4 w-4 ${card.gradient ? 'text-primary-foreground' : card.iconColor}`}
                    />
                  </div>
                  <div>
                    <p className={`text-2xl font-bold font-display ${card.textClass}`}>
                      {card.value}
                    </p>
      ...
      {!hasData ? (
        <Card variant="elevated" className="glass border-border/40 dark:border-glow">
          <CardContent className="py-12">
            <div className="flex flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="font-display font-medium gradient-text">
                Nenhum email enviado no período
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                Os dados aparecerão conforme emails forem enviados
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Tabs defaultValue="trend" className="space-y-4">
          <TabsList className="glass border border-border/50 p-1">
            <TabsTrigger
              value="trend"
              className="gap-2 font-display data-[state=active]:bg-gradient-to-r data-[state=active]:from-primary data-[state=active]:to-accent data-[state=active]:text-primary-foreground"
            >
                    ...
                    >
                      <defs>
                        ...
                        >
                          <stop
                            offset="5%"
                            stopColor="hsl(var(--status-error))"
                            stopOpacity={0.8}
                          />
                          <stop
                            offset="95%"
          ...
          <TabsContent value="functions" className="animate-fade-in">
            <div className="grid lg:grid-cols-2 gap-4">
              <Card
                variant="elevated"
                className="glass border-border/40 dark:border-glow"
              >
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-display font-medium flex items-center gap-2">
                    <div className="p-1.5 rounded-lg bg-gradient-to-br from-primary to-accent shadow-md">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {data?.bySubject.map((s, i) => (
                      <div
                        key={i}
                        className="glass rounded-xl p-4 border border-border/30 hover:border-primary/40 transition-all hover-lift group"
                      >
                        <div className="mb-3">
