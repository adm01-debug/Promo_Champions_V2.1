  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      sales?.forEach(sale => {
        ...
        dealsMap.set(sale.salesperson_id, (dealsMap.get(sale.salesperson_id) || 0) + 1);
      });
    ...
    <Card className="glass border-primary/20 bg-black/40 backdrop-blur-xl relative overflow-hidden group">
      ...
      <CardHeader className="pb-6 border-b border-white/5 relative z-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Badge
            variant="outline"
            className="font-mono text-[9px] font-black uppercase tracking-widest bg-primary/10 border-primary/30 text-primary py-1 px-3 shadow-[0_0_15px_rgba(14,165,233,0.15)]"
          >
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="glass p-4 border-primary/30 rounded-xl shadow-2xl backdrop-blur-xl">
                        ...
                        <p className="text-xl font-mono font-black text-primary italic tracking-tighter">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <div className="h-1 flex-1 bg-white/10 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${(data.revenue / (closers?.[0]?.revenue || 1)) * 100}%`,
                              }}
                            />
                          </div>
        ...
        <div className="grid grid-cols-3 gap-3 mt-8 pt-6 border-t border-white/5">
          {closers?.slice(0, 3).map((closer, index) => (
            ...
            >
              ...
              <p className="text-[10px] font-mono font-bold truncate uppercase tracking-tighter mb-1">
                {closer.name}
              </p>
              <p
                className={`text-xs font-mono font-black ${index === 0 ? 'text-primary' : 'text-foreground/80'}`}
              >
                {closer.deals} DEALS
              </p>
            </div>
