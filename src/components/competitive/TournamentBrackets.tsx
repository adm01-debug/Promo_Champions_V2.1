                    ...
                    <div className="overflow-x-auto">
                      <div className="flex gap-8 min-w-max pb-2">
                        {Array.from({ length: t.total_rounds }, (_, r) => {
                          const roundMatches = matches.filter((m: Record<string, unknown>) => m.round_number === r + 1).sort((a: Record<string, number>, b: Record<string, number>) => a.match_order - b.match_order);
                          ...
                          return (
                            <div key={r} className="flex flex-col gap-4">
                              ...
                              {roundMatches.length > 0 ? roundMatches.map(( match: Record<string, unknown>) => {
                                const p1Name = t.tournament_participants?.find((p: Record<string, string>) => p.salesperson_id === match.player1_id)?.salespeople?.name || 'TBD';
                                const p2Name = match.player2_id ? t.tournament_participants?.find((p: Record<string, string>) => p.salesperson_id === match.player2_id)?.salespeople?.name || 'TBD' : 'BYE';
