            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <Badge
                      variant="outline"
                      className="text-[10px] font-black uppercase tracking-widest bg-primary/10 text-primary border-primary/20"
                    >
              ...
              <div className="flex items-center gap-3">
                <Button
                  ...
                  className="h-10 px-4 border-white/10 text-xs font-black uppercase tracking-widest rounded-xl"
                >
                  <Timer className="h-4 w-4 mr-2" />{' '}
                  {autoRotate ? 'AUTO-BROADCAST' : 'MANUAL'}
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setSoundEnabled(!soundEnabled)}
                  className="h-10 w-10 border-white/10 rounded-xl p-0"
                >
                  {soundEnabled ? (
                ...
                <Button
                  ...
                  className="h-10 px-4 border-white/10 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-primary hover:text-primary-foreground transition-all"
                >
        ...
        <CardContent className="p-10 relative z-10">
          ...
          <div className="text-center mb-12">
            ...
            >
              ...
              <span className="text-4xl font-black italic uppercase tracking-tighter gradient-text leading-none">
                ...
                {salespeople.slice(0, 8).map((sp, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="font-black text-xl italic uppercase tracking-tighter text-foreground">
                        {sp.name}
                      </div>
                      <div className="flex items-center gap-2 mt-1">
                        <Badge
                          variant="outline"
                          className="text-[10px] font-black uppercase tracking-widest bg-black/40 border-white/10"
                        >
                          LVL {sp.level || 1}
                        </Badge>
                    ...
                    <div className="text-right">
                      ...
                      {i === 0 && (
                        <p className="text-[9px] font-black text-rank-gold uppercase tracking-[0.2em] mt-1">
                          Arena Legend
                        </p>
                      )}
                    </div>
                ...
                {latestSales.map((sale, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 relative z-10">
                      <div className="font-black text-2xl italic uppercase tracking-tighter text-foreground leading-none mb-2">
                        {sale.client_name}
                      </div>
                      <div className="flex items-center gap-3">
                        <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/20 text-[10px] font-black uppercase tracking-widest">
                          {sale.product_name}
                        </Badge>
                        ...
                        <span className="text-[10px] text-muted-foreground font-black uppercase tracking-widest flex items-center gap-1.5">
                          ...
                          {spMap.get(sale.salesperson_id ?? '') || 'Elite Closer'}
                        </span>
                      </div>
                    </div>
            ...
            {currentScreen === 'goals' && (
              ...
              >
                {goals.slice(0, 6).map((goal, i: number) => {
                  const goalWithAmount = goal as typeof goal & {
                    current_amount?: number;
                  };
                  const progress =
                    goal.goal_amount > 0
                      ? Math.min(
                          ((goalWithAmount.current_amount || 0) / goal.goal_amount) * 100,
                          100
                        )
                      : 0;
                  return (
                          ...
                          <div className="flex justify-between mt-3 px-1">
                            <div className="flex flex-col">
                              ...
                              <span className="text-sm font-black italic text-foreground tracking-tighter">
                                R${' '}
                                {((goalWithAmount.current_amount || 0) / 1000).toFixed(1)}
                                k
                              </span>
                            </div>
                            <div className="flex flex-col text-right">
                              ...
                              <span className="text-sm font-black italic text-muted-foreground/60 tracking-tighter">
                                R$ {((goal.goal_amount || 0) / 1000).toFixed(1)}k
                              </span>
                            </div>
                          </div>
                        </CardContent>
                ...
                {goals.length === 0 && (
                  <div className="col-span-2 text-center py-12 text-muted-foreground uppercase tracking-widest font-black opacity-20">
                    ...
                    <Card className="glass border-white/5 overflow-hidden group/streak text-center">
                      <CardContent className="p-8">
                        <div className="relative inline-block mb-6">
                          ...
                          {i < 3 && (
                            <div className="absolute -top-2 -right-2 size-6 rounded-full bg-rank-gold flex items-center justify-center border-2 border-background animate-pulse shadow-lg">
                              <Sparkles className="size-3 text-white" />
                            </div>
                          )}
                        </div>
                        <div className="font-black italic uppercase tracking-tighter text-lg truncate mb-2">
                          {sp.name}
                        </div>
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em] opacity-60">
                            {(sp.xp || 0).toLocaleString()} TOTAL XP
                          </div>
                        </div>
                      </CardContent>
                    </Card>
