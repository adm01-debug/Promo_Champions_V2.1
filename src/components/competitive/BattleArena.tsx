...
import { Swords, Trophy, Zap, Plus, Users, TrendingUp } from 'lucide-react';
...
const BattleArenaComponent: FC<BattleArenaProps> = ({ className }) => {
  ...
  const { activeBattles, completedBattles, stats } = useMemo(() => {
    ...
    const totalParticipants = active.reduce(
      (sum: number, b: Battle) => sum + (b.battle_participants?.length || 0),
      0
    );
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <Card
            key={label}
            className={cn(
              'glass border-white/5 overflow-hidden group/stat relative transition-all duration-500 hover:scale-[1.02]',
              border
            )}
          >
            <div
              className={cn(
                'absolute inset-0 opacity-0 group-hover/stat:opacity-100 transition-opacity duration-700',
                bg
              )}
            />
            <CardContent className="p-6 flex items-center gap-5 relative z-10">
              <div
                className={cn(
                  'h-14 w-14 rounded-2xl flex items-center justify-center shrink-0 shadow-2xl transition-all duration-500 group-hover/stat:rotate-12 border border-white/10',
                  bg,
                  color
                )}
              >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="outline"
              size="sm"
              className="h-10 px-6 border-primary/20 bg-primary/10 text-primary hover:bg-primary/20 transition-all text-xs font-black uppercase tracking-widest rounded-xl"
      ...
      {!battles?.length && (
        <Card className="glass border-dashed border-white/10 bg-white/5">
          <CardContent className="p-16 text-center">
            ...
            <p className="text-xl font-black italic uppercase tracking-tighter gradient-text mb-2">
              Arena Deserta
            </p>
            <p className="text-sm text-muted-foreground font-medium mb-8">
              Gestores de elite podem criar duelos para inflamar o time!
            </p>
            <Button
              className="bg-primary/10 text-primary hover:bg-primary/20 border-primary/20 px-8 h-12 text-xs font-black uppercase tracking-[0.2em] rounded-full shadow-2xl shadow-primary/10"
              onClick={() => setShowCreate(true)}
            >
      ...
      {completedBattles.length > 0 && (
        <div className="space-y-6">
          <div className="flex items-center justify-between border-b border-white/5 pb-4">
            ...
            <Badge
              variant="outline"
              className="bg-white/5 border-white/10 text-[10px] font-black uppercase tracking-widest px-4"
            >
