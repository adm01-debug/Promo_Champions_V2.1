...
const PODIUM_CONFIG: Record<
  number,
  { icon: typeof Crown; gradient: string; label: string }
> = {
            ...
            <CardTitle className="text-2xl flex items-center gap-4 italic uppercase font-black tracking-tighter">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge
                    variant="outline"
                    className="text-[10px] font-black uppercase tracking-widest bg-white/5 border-white/10 px-3 py-0.5"
                  >
          ...
          <CardContent className="p-6 space-y-4">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 relative z-10">
                      <div className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
                        <span className="font-black italic uppercase tracking-tighter text-lg text-foreground truncate">
                          {person.name}
                        </span>
                        <Badge
                          variant="outline"
                          className="text-[9px] font-black uppercase tracking-widest bg-black/40 border-white/10 w-fit"
                        >
                          {config?.label}
                        </Badge>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="flex-1 h-2 bg-black/20 rounded-full overflow-hidden border border-white/5">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{
                              width: `${(person.weeklySales / maxSales) * 100}%`,
                            }}
                            transition={{ duration: 1, ease: 'circOut' }}
                            className={cn(
                              'h-full relative',
                              i === 0 ? 'bg-rank-gold' : 'bg-primary'
                            )}
                          >
            ...
            {rest.length > 0 && (
              <div className="space-y-3 pt-6 border-t border-white/5">
                ...
                {rest.map((person, idx) => (
                  ...
                  >
                    <span className="text-xs font-black italic text-muted-foreground w-6 text-center group-hover/rest:text-primary transition-colors">
                      #{person.rank}
                    </span>
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-sm font-bold uppercase tracking-tight text-foreground flex-1 truncate">
                        {person.name}
                      </span>
            ...
            {(!ranking?.length || ranking[0].weeklySales === 0) && (
              <div className="text-center py-12 relative overflow-hidden rounded-3xl bg-white/5 border border-dashed border-white/10">
                ...
                <p className="text-lg font-black italic uppercase tracking-tighter gradient-text">
                  A Arena está Aberta
                </p>
                <p className="text-xs text-muted-foreground font-medium mt-1 uppercase tracking-widest px-8">
                  Nenhuma venda registrada esta semana. Quem conquistará o trono primeiro?
                  🚀
                </p>
              </div>
            )}
