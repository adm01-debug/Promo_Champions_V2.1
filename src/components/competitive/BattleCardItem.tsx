        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <Badge
                  variant="outline"
                  className="text-[9px] font-black uppercase tracking-widest bg-black/40 border-white/10 text-muted-foreground"
                >
                  {battle.metric}
                </Badge>
                <Badge
          ...
          <div className="flex items-center md:flex-col md:items-end gap-3">
            {isActive ? (
              <Badge
                variant="outline"
                className="text-[10px] font-black uppercase tracking-widest bg-rose-500/20 text-rose-400 border-rose-500/30 px-4 py-1.5 animate-pulse shadow-lg shadow-rose-500/10"
              >
              ...
              <Badge
                variant="outline"
                className="text-[10px] font-black uppercase tracking-widest bg-white/10 text-muted-foreground border-white/10 px-4 py-1.5"
              >
                FINALIZADO
              </Badge>
            )}
                  ...
                  <div className="flex items-center gap-5">
                    <div
                      className={cn(
                        ...
                        i === 0 && isActive
                          ? 'bg-rank-gold text-white border-rank-gold/50 shadow-rank-gold/20'
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-end mb-2">
                        <div className="flex items-center gap-2">
                          <span className="text-base font-black italic uppercase tracking-tighter truncate group-hover/player:text-primary transition-colors">
                            {p.salespeople?.name || 'Gladiador'}
                          </span>
                          {i === 0 && isActive && (
                            <Sparkles className="size-3 text-rank-gold animate-pulse" />
                          )}
                        </div>
        ...
        {!isActive && battle.winner_id && (
          <div className="mt-6 p-6 rounded-3xl bg-gradient-to-br from-rank-gold/20 via-rank-gold/5 to-transparent border border-rank-gold/30 text-center relative overflow-hidden group/winner shadow-2xl shadow-rank-gold/10">
            ...
            <div className="relative z-10">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-rank-gold mb-2">
              ...
              <p className="text-3xl font-black italic uppercase tracking-tighter text-foreground drop-shadow-lg">
                {participants.find(
                  p =>
                    p.salespeople?.name &&
                    (p.id === battle.winner_id || p.salespeople.name === battle.winner_id)
                )?.salespeople?.name || 'Elite Legend'}
              </p>
            </div>
          </div>
        )}
