...
export function TargetSimulator() {
  ...
  const projectedMeetings = Math.round(
    currentLeads * (1 + activityIncrease / 100) * (currentConv / 100)
  );
      ...
      <CardContent className="pt-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-xl bg-primary/10 border border-primary/20 text-center space-y-1">
            ...
            <p className="text-[10px] uppercase tracking-tighter text-primary font-bold">
              Comissão Extra
            </p>
            <p className="text-xl font-bold text-primary">
              R$ {extraCommission.toLocaleString()}
            </p>
        ...
        <div className="p-3 rounded-lg bg-success/10 border border-success/20">
          <p className="text-[10px] text-success-foreground text-center font-medium">
            "Com +{activityIncrease}% de esforço, você atinge o próximo acelerador de
            1.5x!"
          </p>
        </div>
      </CardContent>
