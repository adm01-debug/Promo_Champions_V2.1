        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2
                className={cn(
                  'text-xl font-mono font-black uppercase tracking-widest',
                  getRateColor(rate)
                )}
              >
                {title}
          ...
          <div className="flex flex-col items-end gap-1">
            <Badge
              variant="outline"
              className="gap-2 px-3 py-1 text-[9px] font-mono font-black uppercase tracking-widest border-primary/30 bg-primary/10 text-primary"
            >
              <Zap className="h-3 w-3" />
              CORE_METRIC
            </Badge>
            <span className="text-[8px] font-mono text-muted-foreground/40">
              SYS_ID: {Math.random().toString(16).slice(2, 8).toUpperCase()}
            </span>
          </div>
        </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-[10px] font-mono font-bold uppercase tracking-widest">
                ...
                <span className={getRateColor(rate)}>
                  {rate >= 15
                    ? `ALPHA: +${(rate - 15).toFixed(1)}%`
                    : `DELTA: ${(15 - rate).toFixed(1)}%`}
                </span>
              </div>
              <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden border border-white/10">
                <motion.div
                  ...
                  className={cn(
                    'h-full shadow-[0_0_10px_currentColor] relative overflow-hidden',
                    getRateColor(rate).replace('text-', 'bg-')
                  )}
                >
                  <motion.div
