...
export function SDRActivityTrend({ period }: SDRActivityTrendProps) {
  ...
  const { data, isLoading } = useSDRActivityTrend(
    period,
    selectedSDR,
    viewMode,
    activityTypeFilter
  ...
  const totalActivities = totals
    ? totals.calls + totals.emails + totals.meetings + totals.linkedin + totals.whatsapp
    : 0;
  ...
  return (
    <Card className="glass border-primary/20 bg-black/40 backdrop-blur-xl relative overflow-hidden group h-full">
      ...
      <CardHeader className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 pb-6 border-b border-white/5 relative z-10">
        <div className="flex items-center gap-4">
          ...
          {totalActivities > 0 && viewMode === 'activity' && (
            <Badge
              variant="outline"
              className="font-mono text-[9px] uppercase tracking-widest bg-white/5 border-white/10"
            >
              VOL: {totalActivities}
            </Badge>
          )}
            ...
            <TooltipProvider>
              <UITooltip>
                <TooltipTrigger asChild>
                  <Badge
                    variant="destructive"
                    className="gap-1.5 cursor-help animate-pulse text-[9px] uppercase tracking-widest"
                  >
                ...
                <TooltipContent
                  side="bottom"
                  className="max-w-xs bg-black/90 border-destructive/30 backdrop-blur-xl"
                >
                  <div className="space-y-2 p-1">
                    <p className="font-mono font-bold text-[10px] text-destructive uppercase tracking-widest">
                      Alert: Performance GAP Detected
                    </p>
                    <ul className="space-y-1">
                      {underperformingSDRs.map(sdr => (
                        <li
                          key={sdr.id}
                          className="text-[10px] font-mono flex items-center justify-between gap-3"
                        >
                          <span className="text-muted-foreground">{sdr.name}</span>
                          <span className="text-destructive font-black">
                            {sdr.consecutiveDays}D | -{sdr.avgDeficit}/DAY
                          </span>
                        </li>
                      ))}
                    </ul>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <TabsList className="h-8 bg-black/40 border border-white/5 p-1">
              <TabsTrigger
                value="activity"
                className="text-[9px] font-black uppercase tracking-tighter px-2 h-6"
              >
                MODE: TYPE
              </TabsTrigger>
              <TabsTrigger
            ...
            {viewMode === 'activity' ? (
              <Select value={selectedSDR} onValueChange={setSelectedSDR}>
                ...
                <SelectContent className="bg-black/90 border-white/10 backdrop-blur-xl">
                  <SelectItem value="all" className="text-[10px] font-mono uppercase">
                    Global Spectrum
                  </SelectItem>
                  {sdrs.map(sdr => (
                    <SelectItem
              ...
              <Select
                ...
                onValueChange={v => setActivityTypeFilter(v as ActivityTypeFilter)}
              >
                <SelectTrigger className="w-[140px] h-8 text-[9px] font-mono font-bold uppercase tracking-widest bg-black/40 border-white/10">
      ...
      <CardContent className="pt-8 relative z-10">
        <div
          className="absolute inset-x-6 top-0 bottom-6 opacity-[0.03] pointer-events-none border border-white/10 rounded-xl"
          style={{
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
                  ...
                  return (
                    <div className="glass p-4 border-white/10 rounded-xl shadow-2xl backdrop-blur-xl min-w-[180px]">
                      <div className="flex items-center justify-between gap-4 mb-3 border-b border-white/5 pb-2">
                        <p className="font-mono font-black text-[10px] text-foreground uppercase tracking-widest">
                          {label}
                        </p>
                        <Badge
                          variant="outline"
                        ...
                        {payload.map(entry => (
                          ...
                          >
                            ...
                            <span className="text-[9px] font-mono font-bold text-muted-foreground uppercase tracking-wider">
                              {ACTIVITY_LABELS[String(entry.dataKey)] ||
              ...
              <Legend
                ...
                iconSize={8}
                formatter={value => (
                  <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors">
                    {ACTIVITY_LABELS[value] || value}
                  </span>
                )}
              />
              {(['calls', 'emails', 'meetings', 'linkedin', 'whatsapp'] as const).map(
                key => (
                  <Area
                    key={key}
                    type="monotone"
            ...
            >
                            ...
                            return (
                              <div key={dk} className="space-y-1">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className={`text-[10px] font-mono font-black ml-auto ${goal > 0 ? (diff >= 0 ? 'text-success' : 'text-destructive') : ''}`}
                                  >
                                    {entryVal}
                                  </span>
                                </div>
                                  ...
                                  <div className="flex items-center gap-1.5 ml-3.5">
                                    <div className="h-0.5 flex-1 bg-white/5 rounded-full overflow-hidden">
                                      <div
                                        className={cn(
                                          'h-full',
                                          diff >= 0 ? 'bg-success' : 'bg-destructive'
                                        )}
                                        style={{
                                          width: `${Math.min((entryVal / goal) * 100, 100)}%`,
                                        }}
                                      />
                                    </div>
                                    <span
              ...
              {sdrs.map((sdr, index) => (
                <Line
                  ...
                  strokeWidth={3}
                  dot={{ fill: COLORS[index % COLORS.length], strokeWidth: 0, r: 0 }}
