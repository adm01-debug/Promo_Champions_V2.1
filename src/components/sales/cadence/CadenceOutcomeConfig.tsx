...
interface OutcomeRule extends OutcomeRuleForm {
  id: string;
  created_at?: string;
}
interface AlertTemplateOption {
  id: string;
  name: string;
  type: string;
}
...
export function CadenceOutcomeConfig() {
  ...
  const handleSave = async () => {
    try {
      const { error } = await supabase.from('cadence_outcome_rules').insert([formData]);
          ...
          <CardContent className="space-y-4">
              ...
              <div className="p-4 rounded-xl border border-primary/20 bg-primary/5 space-y-4 animate-in fade-in slide-in-from-top-2">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={v => setFormData({ ...formData, to_stage: v })}
                    >
                      <SelectTrigger className="h-8 text-xs">
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={v => setFormData({ ...formData, next_action: v })}
                    >
                      <SelectTrigger className="h-8 text-xs">
                        <SelectValue />
                      </SelectTrigger>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        onChange={e =>
                          setFormData({
                            ...formData,
                            retry_delay_hours: parseInt(e.target.value),
                          })
                        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-primary/10">
                  <div className="space-y-1.5">
                    ...
                    <Select
                      value={formData.push_template_id || 'none'}
                      onValueChange={v =>
                        setFormData({
                          ...formData,
                          push_template_id: v === 'none' ? null : v,
                        })
                      }
                    >
                      <SelectTrigger className="h-8 text-xs bg-background">
            ...
            <div className="space-y-2">
              {rules.map(rule => (
                <div
                  key={rule.id}
                  className="flex items-center justify-between p-3 rounded-xl border border-border/30 bg-muted/10 group"
                >
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      <Phone className="h-3 w-3 text-muted-foreground" />
                      <span className="text-xs font-bold uppercase">
                        {OUTCOMES.find(o => o.id === rule.outcome)?.label || rule.outcome}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs">
                        {STAGES.find(s => s.id === rule.to_stage)?.label || rule.to_stage}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-muted-foreground uppercase">
                        {rule.next_action === 'retry'
                          ? `Tentar em ${rule.retry_delay_hours}h (Max ${rule.max_retries})`
                          : rule.next_action}
                      </span>
                    </div>
