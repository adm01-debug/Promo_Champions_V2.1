...
import {
  usePurchaseHeatmap,
  type HeatmapCell,
} from '@/hooks/purchase-intelligence/usePurchaseIntelligence';
import {
  intensityColor,
  formatBRL,
  monthLabelFromIso,
...
function buildMatrix(rows: HeatmapCell[]): {
  ...
  for (const r of rows) {
    ...
    if (r.is_current_user) {
      ...
      cell.hasOthers = true;
      cell.others.push({
        name: r.salesperson_name ?? 'Outro vendedor',
  ...
  const clients = Array.from(byClient.entries()).map(([id, c]) => {
    ...
    cellsArray.forEach(cell => {
      if (cell.totalRevenue > avgMonthly * 2.5 && cell.totalRevenue > 5000)
        cell.anomaly = 'high';
        ...
        <CardContent className="py-20 text-center space-y-4">
          ...
          <div className="space-y-1">
            <p className="text-muted-foreground font-bold text-lg">
              Sem histórico térmico
            </p>
            <p className="text-sm text-muted-foreground max-w-[280px]">
              Não encontramos registros de compras para os parâmetros selecionados nos
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <CardTitle className="text-section-title flex items-center gap-2.5">
              ...
              <Badge
                variant="outline"
                className="ml-2 font-mono text-[10px] uppercase tracking-tighter border-primary/20 bg-primary/5"
              >
                {matrix.months.length} Meses
              </Badge>
            </CardTitle>
            <p className="text-xs text-muted-foreground">
              Analise a densidade de compras e identifique janelas de oportunidade tática.
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-6 mt-4 pt-2 border-t border-border/40">
          <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
            <div className="flex items-center gap-1.5">
              <div className="h-3 w-3 rounded-[3px] border-2 border-primary bg-primary/20" />{' '}
              Minhas Compras
            </div>
            <div className="flex items-center gap-1.5">
              <div className="h-3 w-3 rounded-[3px] border-2 border-dashed border-orange-500 bg-orange-500/10" />{' '}
              Mercado/Outros
            </div>
            <div className="flex items-center gap-1.5">
              <div className="h-3 w-3 rounded-[3px] bg-red-500 animate-pulse" /> Anomalia
              Alta
            </div>
          </div>
              ...
              <thead>
                <tr>
                  ...
                  {matrix.months.map(m => (
                    <th
                      key={m}
                      className="text-[10px] font-bold text-muted-foreground pb-8 min-w-[50px] relative"
                    >
                ...
                {filteredClients.slice(0, isFullscreen ? 100 : 25).map((c, idx) => (
                  ...
                  >
                    ...
                    {matrix.months.map(m => {
                      ...
                      const bg = cell
                        ? intensityColor(cell.totalRevenue, matrix.max)
                        : 'hsl(var(--muted) / 0.15)';
                          ...
                          <Tooltip>
                                ...
                                {cell?.anomaly && (
                                  <div
                                    className={cn(
                                      ...
                                      cell.anomaly === 'high'
                                        ? 'bg-red-500 animate-pulse shadow-[0_0_5px_red]'
                                        : 'bg-blue-400'
                                    )}
                                  />
                                )}
                            ...
                            <AnimatePresence>
                              {cell && (
                                <TooltipContent
                                  side="top"
                                  className="p-0 border-none bg-transparent shadow-2xl overflow-hidden"
                                  sideOffset={10}
                                >
                                  <motion.div
                                    ...
                                    {cell.others.length > 0 && (
                                      <div className="space-y-1.5 pt-1 border-t border-border/50">
                                        <div className="text-[9px] font-bold text-orange-500 uppercase tracking-widest flex items-center gap-1">
                                          <AlertCircle className="h-3 w-3" /> Outros
                                          Vendedores
                                        </div>
                                        <div className="space-y-1">
        ...
        <div className="p-4 bg-muted/20 border-t border-border/50 flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5">
              <Info className="h-3.5 w-3.5" /> Mostrando{' '}
              {Math.min(filteredClients.length, isFullscreen ? 100 : 25)} de{' '}
