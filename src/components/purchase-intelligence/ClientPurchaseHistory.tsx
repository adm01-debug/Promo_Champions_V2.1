...
import { usePurchaseHistory, type OrderWithDetails } from "@/hooks/orders/usePurchaseHistory";
import { formatBRL, formatDatePt } from "../purchase-intelligence/purchaseIntelligenceHelpers";
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <CardTitle className="flex items-center gap-2.5 text-lg font-bold">
              ...
              <Badge variant="secondary" className="ml-2 font-mono text-[10px] bg-primary/5 text-primary border-primary/20">
                {filteredOrders.length} REGISTROS
              </Badge>
            </CardTitle>
            <p className="text-xs text-muted-foreground">Rastreabilidade completa de transações, itens e performance de venda.</p>
          </div>
              ...
              >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge 
                          ...
                          className={cn(
                            ...
                            statusTone(order.status) === "success" && "bg-success/10 text-success border-success/20",
                            statusTone(order.status) === "warning" && "bg-warning/10 text-warning border-warning/20",
                            statusTone(order.status) === "destructive" && "bg-destructive/10 text-destructive border-destructive/20",
                            statusTone(order.status) === "info" && "bg-info/10 text-info border-info/20"
                          )}
                        >
                      ...
                      <div className="flex items-center gap-3 text-[10px] font-medium text-muted-foreground">
                        <span className="flex items-center gap-1"><Calendar className="h-3 w-3" /> {formatDatePt(order.created_at)}</span>
                        <span className="flex items-center gap-1"><User className="h-3 w-3" /> {order.seller_name}</span>
                      </div>
                    </div>
                  </div>

                  <div className="text-right space-y-1">
                    <div className="text-sm font-black text-foreground">{formatBRL(order.total)}</div>
                ...
                <div className="mt-3 flex gap-2 overflow-hidden">
                  {order.items.slice(0, 3).map((item, idx) => (
                    <Badge key={idx} variant="secondary" className="text-[9px] bg-muted/30 text-muted-foreground border-none font-normal">
                      {item.product_name}
                    </Badge>
                  ))}
                  {order.items.length > 3 && (
                    <Badge variant="secondary" className="text-[9px] bg-muted/30 text-muted-foreground border-none font-normal">
                      +{order.items.length - 3}
                    </Badge>
                  )}
                </div>
            ...
            <>
              <div className="h-24 bg-gradient-to-r from-primary/20 via-primary/5 to-transparent relative">
                <div className="absolute bottom-4 left-6 flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-white tracking-tight">Pedido #{selectedOrder.order_number}</h3>
                    <div className="flex items-center gap-2">
                      <Badge className="bg-primary/10 text-primary border-primary/20 h-5 text-[10px] font-bold">DETALHES DA TRANSAÇÃO</Badge>
                      <span className="text-[10px] text-muted-foreground font-mono">{selectedOrder.id}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <DetailStat label="Data" value={formatDatePt(selectedOrder.created_at)} icon={<Calendar className="h-3 w-3" />} />
                  <DetailStat label="Vendedor" value={selectedOrder.seller_name || "—"} icon={<User className="h-3 w-3" />} />
                  <DetailStat label="Status" value={statusLabel(selectedOrder.status)} icon={<Package className="h-3 w-3" />} />
                  <DetailStat label="Subtotal" value={formatBRL(selectedOrder.subtotal)} icon={<TrendingUp className="h-3 w-3" />} />
                </div>
                    ...
                    {selectedOrder.items.map((item, idx) => (
                      <div key={idx} className="p-3 flex items-center justify-between text-sm">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-muted-foreground">{item.quantity} un × {formatBRL(item.unit_price)}</div>
                        </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Resumo Financeiro</h4>
                    <div className="p-4 rounded-xl bg-muted/40 border border-border/50 space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Subtotal</span>
                        <span className="font-medium">{formatBRL(selectedOrder.subtotal)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Frete</span>
                        <span className="font-medium">{formatBRL(selectedOrder.shipping)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Descontos</span>
                        <span className="text-success font-medium">-{formatBRL(selectedOrder.discount_amount || 0)}</span>
                      </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Análise de Performance</h4>
                    <div className="p-4 rounded-xl bg-primary/5 border border-primary/20 space-y-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="text-[10px] font-bold text-muted-foreground uppercase">Impacto no LTV</div>
                          <div className="text-sm font-bold text-foreground">+12.4% no ticket médio</div>
                        </div>
                      </div>
                      <div className="flex items-start gap-3">
                        <div className="p-1.5 rounded-lg bg-primary/20 text-primary">
                        ...
                        <div>
                          <div className="text-[10px] font-bold text-muted-foreground uppercase">Janela de Conversão</div>
                          <div className="text-sm font-bold text-foreground">Fechamento em 4 dias (Fast-track)</div>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="p-4 bg-muted/30 border-t border-border/50 flex justify-end gap-3">
                <Button variant="outline" size="sm" onClick={() => setSelectedOrder(null)}>Fechar</Button>
