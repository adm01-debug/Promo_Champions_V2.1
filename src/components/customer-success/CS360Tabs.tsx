  ...
  return (
    <>
      <TabsList className="flex flex-wrap h-auto bg-muted/50 p-1 rounded-xl">
        ...
        <TabsTrigger value="cohorts" className="rounded-lg">
          Coortes
        </TabsTrigger>
        <TabsTrigger value="health" className="rounded-lg">
          Health v2
        </TabsTrigger>
        <TabsTrigger value="renewals" className="rounded-lg">
          Renovações
        </TabsTrigger>
        ...
        <TabsTrigger value="expansion" className="rounded-lg">
          Expansion
        </TabsTrigger>
        <TabsTrigger value="surveys" className="rounded-lg">
          CSAT/CES
        </TabsTrigger>
        <TabsTrigger value="qbr" className="rounded-lg">
          QBR
        </TabsTrigger>
      ...
      <TabsContent value="overview" className="space-y-4 mt-4">
        <Card className="glass border-border/50">
          ...
          <CardContent className="space-y-2">
            {accounts
              .filter(a => a.health_v2 < 50)
              .slice(0, 10)
              .map(a => (
                <div
                  key={a.id}
                  className="flex items-center justify-between border border-border/40 rounded-lg p-3 hover:bg-muted/30 transition-colors"
                >
                  <div>
                    <div className="font-semibold text-sm">{a.name}</div>
                    <div className="text-[10px] text-muted-foreground">
                      {a.tier} • {a.open_tickets} tickets • Renovação:{' '}
                      {a.next_renewal ?? '—'}
                    </div>
                  </div>
        ...
        <Card className="glass border-border/50">
          ...
          <CardContent className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {accounts.slice(0, 30).map(a => (
              <div
                key={a.id}
                className="border border-border/40 rounded-lg p-3 space-y-2 hover:bg-muted/30 transition-colors"
              >
            ...
            {renewals.length === 0 ? (
              ...
              renewals.map(r => {
                ...
                return (
                  <div
                    key={r.id}
                    className={`border rounded-lg p-3 flex items-center justify-between transition-all hover:translate-x-1 ${SEMA_BG[sema]}`}
                  >
                    <div>
                      ...
                      <div className="text-[10px] opacity-80 font-mono uppercase">
                        {r.renewal_date} • {d !== null ? `${d}d` : '—'} •{' '}
                        {RENEWAL_STATUS_LABEL[r.status]}
                      </div>
                    </div>
                    <div className="font-bold text-sm">
                      {formatBRL(Number(r.contract_value))}
                    </div>
                  </div>
                );
              })
          ...
          <CardContent className="space-y-2">
            {tickets.length === 0 ? (
              ...
              tickets.map(t => (
                <div
                  key={t.id}
                  className="border border-border/40 rounded-lg p-3 flex items-center justify-between hover:bg-muted/30 transition-colors"
                >
          ...
          <CardContent className="grid gap-3 md:grid-cols-2">
            {usage.length === 0 ? (
              ...
              usage.slice(0, 30).map(u => (
                <div
                  key={u.account_id}
                  className="border border-border/40 rounded-lg p-3 hover:bg-muted/30 transition-colors"
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-semibold text-sm">
                      {accountById.get(u.account_id)?.name ?? '—'}
                    </span>
          ...
          <CardContent className="grid gap-3 md:grid-cols-2">
            {onboarding.length === 0 ? (
              ...
              onboarding.map(o => {
                const pct =
                  o.total_steps > 0
                    ? Math.round((o.current_step / o.total_steps) * 100)
                    : 0;
                return (
                  <div
                    key={o.id}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge
                        variant={o.status === 'stalled' ? 'destructive' : 'secondary'}
                        className="text-[10px] uppercase font-mono"
                      >
                        {ONBOARDING_STATUS_LABEL[o.status]}
                      </Badge>
                    </div>
                    <div className="flex justify-between text-[10px] text-muted-foreground mb-1">
                      <span>Progresso</span>
                      <span>
                        {o.current_step} / {o.total_steps}
                      </span>
                    </div>
            ...
            {expansion.length === 0 ? (
              ...
              expansion.map(e => (
                ...
                >
                  <div>
                    <div className="font-semibold text-sm">
                      {accountById.get(e.account_id)?.name ?? '—'}
                    </div>
                    <div className="text-[10px] text-muted-foreground uppercase">
                      {EXPANSION_TYPE_LABEL[e.type]} • Confiança: {e.confidence_score}%
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-bold text-success text-sm">
                      {formatBRL(Number(e.estimated_value))}
                    </div>
                    <Badge
                      variant="outline"
        ...
        <Card className="glass border-border/50">
          ...
          <CardContent className="space-y-2">
            {surveys.filter(sv => sv.responded_at).length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">
                Nenhuma resposta de CSAT/CES ainda.
              </p>
            ) : (
              surveys
                .filter(sv => sv.responded_at)
                .slice(0, 30)
                .map(sv => (
                  <div
                    key={sv.id}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-sm">
                        {sv.account_id
                          ? (accountById.get(sv.account_id)?.name ?? '—')
                          : '—'}
                      </span>
            ...
            {qbrs.length === 0 ? (
              ...
              qbrs.map(q => (
                ...
                >
                  <div>
                    <div className="font-semibold text-sm">
                      {accountById.get(q.account_id)?.name ?? '—'}
                    </div>
                    <div className="text-[10px] text-muted-foreground uppercase">
                      Frequência: {q.frequency} • Último: {q.last_qbr_at ?? '—'}
                    </div>
                  </div>
                  <Badge variant="outline" className="text-[10px] font-mono">
                    {q.next_qbr_at ?? 'Não agendado'}
                  </Badge>
                </div>
              ))
            )}
