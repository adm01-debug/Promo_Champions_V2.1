...
export const CustomerSuccess360Hub = memo(function CustomerSuccess360Hub() {
  ...
  const [period, setPeriod] = useState(
    () => localStorage.getItem(`${STORAGE_KEY}_period`) || '30'
  );
  const [startDate, setStartDate] = useState(
    () => localStorage.getItem(`${STORAGE_KEY}_startDate`) || ''
  );
  const [endDate, setEndDate] = useState(
    () => localStorage.getItem(`${STORAGE_KEY}_endDate`) || ''
  );
  const [orderModalStatus, setOrderModalStatus] = useState<string | null>(
    () => localStorage.getItem(`${STORAGE_KEY}_modalStatus`) || null
  );
  const [orderSearch, setOrderSearch] = useState(
    () => localStorage.getItem(`${STORAGE_KEY}_orderSearch`) || ''
  );
  const [orderPage, setOrderPage] = useState(
    () => Number(localStorage.getItem(`${STORAGE_KEY}_orderPage`)) || 1
  );
  const [orderSortField, setOrderSortField] = useState<string>(
    () => localStorage.getItem(`${STORAGE_KEY}_sortField`) || 'created_at'
  );
  const [orderSortOrder, setOrderSortOrder] = useState<'asc' | 'desc'>(
    () => (localStorage.getItem(`${STORAGE_KEY}_sortOrder`) as 'asc' | 'desc') || 'desc'
  );
  const orderItemsPerPage = 10;

  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY}_period`, period);
  }, [period]);
  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY}_startDate`, startDate);
  }, [startDate]);
  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY}_endDate`, endDate);
  }, [endDate]);
  useEffect(() => {
    if (orderModalStatus)
      localStorage.setItem(`${STORAGE_KEY}_modalStatus`, orderModalStatus);
    else localStorage.removeItem(`${STORAGE_KEY}_modalStatus`);
  }, [orderModalStatus]);
  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY}_orderSearch`, orderSearch);
  }, [orderSearch]);
  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY}_orderPage`, orderPage.toString());
  }, [orderPage]);
  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY}_sortField`, orderSortField);
  }, [orderSortField]);
  useEffect(() => {
    localStorage.setItem(`${STORAGE_KEY}_sortOrder`, orderSortOrder);
  }, [orderSortOrder]);
  ...
  const cohortData = useMemo(() => {
    const cohorts: Record<
      string,
      { month: string; retained: number; churned: number; revenue: number }
    > = {};
    ...
    accounts.forEach(a => {
      ...
      if (!cohorts[month])
        cohorts[month] = { month, retained: 0, churned: 0, revenue: 0 };
  ...
  const ordersByStatus = useMemo(() => {
    const statusMap: Record<
      string,
      { count: number; value: number; color: string; status: string; key: string }
    > = {
      delivered: {
  ...
  const exportPDF = () => {
    ...
    doc.autoTable({
      head: [['Conta', 'Tier', 'Health', 'ARR', 'Renovação']],
      body: tableData,
      startY: 20,
    });
  ...
  return (
    <motion.div {...fadeIn} className="space-y-6 container mx-auto p-4 md:p-6 lg:p-8">
      <Helmet>
        <title>CS 360 Hub | Promo Champions</title>
      </Helmet>
