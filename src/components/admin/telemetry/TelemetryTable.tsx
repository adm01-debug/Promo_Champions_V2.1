...
export const TelemetryTable = React.memo(function TelemetryTable({ rows, isLoading }: TelemetryTableProps) {
  return (
    <Card>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="p-3 text-xs text-muted-foreground max-w-[200px] truncate" title={row.error_message || ""}>{row.error_message || "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
