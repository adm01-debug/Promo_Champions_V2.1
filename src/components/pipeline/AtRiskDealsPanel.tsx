...
import {
  useAtRiskDeals,
  useAnalyzeAtRiskDeal,
} from '@/hooks/deal-intelligence/useAtRiskDeals';
...
export function AtRiskDealsPanel() {
    ...
    return (
      ...
      >
        ...
        <CardContent className="space-y-2">
          <Skeleton
            className="h-16 w-full animate-shimmer"
  ...
  return (
    <Card
      variant="elevated"
      className="glass border-border/40 dark:border-glow card-elevated transition-all duration-300 animate-fade-in"
    >
        ...
        <div className="flex gap-4 mt-3 p-2.5 rounded-lg glass border border-border/30 shadow-inner">
          ...
          {data.summary.high > 0 && (
            ...
            >
              ...
              <span className="text-xs">
                <span className="font-bold text-status-warning">{data.summary.high}</span>{' '}
                altos
              </span>
            </div>
          )}
          {data.summary.medium > 0 && (
            ...
            >
              ...
              <span className="text-xs">
                <span className="font-bold text-rank-gold">{data.summary.medium}</span>{' '}
                médios
              </span>
            </div>
          )}
          {data.summary.totalValueAtRisk > 0 && (
            <div
              className="ml-auto text-xs text-muted-foreground animate-fade-in"
              style={{ animationDelay: '150ms' }}
            >
              Valor em risco:{' '}
              <span className="font-display font-bold text-status-error">
                {formatCurrency(data.summary.totalValueAtRisk)}
              </span>
            </div>
          )}
        </div>
