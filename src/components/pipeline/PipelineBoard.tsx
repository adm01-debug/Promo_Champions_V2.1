...
export const PipelineBoard = () => {
  ...
  const {
    ...
    isRefetching: multiRefetching,
  } = usePipelineDealsByPipeline(selectedPipelineId, dynamicStages || []);
  ...
  const leadScores = useMemo(() => {
    ...
    const mapped: Record<
      string,
      { score: number; category: 'hot' | 'warm' | 'cold'; factors: string[] }
    > = {};
  ...
  const weightedTotalValue = currentStages.reduce((sum, stage) => {
    const stageValue =
      (currentDealsByStage?.[stage.id]?.reduce((s, d) => s + d.amount, 0) || 0) *
      (1 + ticketSimulation / 100);
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 p-3 bg-card/60 backdrop-blur-xl border border-border/20 rounded-2xl shadow-2xl">
          ...
          <Button
            variant="ghost"
            size="sm"
            className="h-8 text-[10px] font-bold uppercase tracking-tighter hover:bg-primary/5"
          >
            Alta Prioridade
          </Button>
          <Button
            ...
            className={cn(
              ...
              showFunnelLayer ? 'bg-primary/20' : ''
            )}
            onClick={() => setShowFunnelLayer(!showFunnelLayer)}
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <span
                  className={cn(
                    ...
                    ticketSimulation >= 0 ? 'text-emerald-500' : 'text-red-500'
                  )}
                >
                  {ticketSimulation > 0 ? '+' : ''}
                  {ticketSimulation}%
        ...
        <div className="relative flex items-center justify-between p-6 rounded-2xl bg-gradient-to-r from-card/80 via-card/40 to-background/50 backdrop-blur-xl border border-border/20 shadow-2xl overflow-hidden group">
          ...
          <div className="flex gap-12 relative z-10">
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-[0.2em] mb-1">
                Pipeline Health (P2)
              </span>
            ...
            {isDefaultPipeline && (
              ...
              >
                <Zap
                  className={cn(
                    'h-3.5 w-3.5 mr-2',
                    calculateScores.isPending ? 'animate-pulse' : 'fill-current'
                  )}
                />
                Analisar Leads
              </Button>
        ...
        >
          <div className="flex gap-4 overflow-x-auto pb-4 scroll-smooth snap-x snap-mandatory md:snap-none 2xl:grid 2xl:grid-cols-7 2xl:overflow-x-visible">
            {currentStages.map((stage, index) => {
              ...
              const dealsInNextStage = nextStage
                ? currentDealsByStage?.[nextStage.id]?.length || 0
                : 0;
