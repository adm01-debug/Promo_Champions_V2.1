...
} from '@/hooks/win-loss/useWinLossScenarios';
...
const PI_LEVEL_OPTIONS: ReadonlyArray<{
  level: ConfidenceLevel;
  label: string;
  hint: string;
}> = [
  { level: 0.9, label: '90%', hint: 'Banda mais estreita · maior risco de cobertura' },
...
function readConfidenceLevel(): ConfidenceLevel {
  ...
  if (!raw) return 0.95;
  const n = Number(raw);
  if (n === 0.9 || n === 0.95 || n === 0.99) return n;
  return 0.95;
}
      ...
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span
          className={`text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded ${
            isForecast ? 'bg-primary/15 text-primary' : 'bg-muted text-muted-foreground'
          }`}
        >
...
function ActiveFormulaBadge({
  ...
  const formula = isPi ? `PI ${levelPct}% · ±t·σ·√(1+1/n+(x−x̄)²/Sxx)` : 'SEE · ±z·σ';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 text-[10px] font-medium text-foreground leading-tight">
        ...
        <span className="truncate" title={formula}>
          {formula}
        </span>
      </div>
      ...
      <Card>
        ...
        <CardContent className="h-[260px] flex items-center justify-center">
          <p className="text-sm text-muted-foreground text-center max-w-[260px]">
            Necessários ao menos <strong>3 períodos</strong> com dados para projeção
            confiável. Ajuste os filtros ou aguarde mais histórico.
          </p>
        </CardContent>
      </Card>
    );
                ...
                <UITooltip>
                  <TooltipTrigger asChild>
                    <ToggleGroupItem
                      ...
                      aria-label="Modo SEE (1σ)"
                    >
                      SEE
                    </ToggleGroupItem>
                  </TooltipTrigger>
                  <TooltipContent side="top" className="text-xs max-w-[220px]">
                    Banda 1σ via prediction interval · σ · √(1 + 1/n + (x − x̄)² / Sxx).
                    ~68% de confiança, abre com horizonte.
                  </TooltipContent>
                </UITooltip>
                <UITooltip>
                  <TooltipTrigger asChild>
                    <ToggleGroupItem
                      value="pi95"
                      className="h-6 px-2 text-[10px] font-medium"
                      aria-label="Modo PI 95%"
                    >
                      PI 95%
                    </ToggleGroupItem>
                  </TooltipTrigger>
                  <TooltipContent side="top" className="text-xs max-w-[240px]">
                    Intervalo de previsão com multiplicador t-Student no nível escolhido
                    (90/95/99%). Mais conservador, leva em conta a distância do centro dos
                    dados.
                  </TooltipContent>
                </UITooltip>
              </ToggleGroup>
              {bandMode === 'pi95' && (
                <Popover>
                  <UITooltip>
                    ...
                    <TooltipContent side="top" className="text-xs max-w-[220px]">
                      Nível de confiança do intervalo de previsão. t-Student é recalculado
                      conforme gl.
                    </TooltipContent>
                  </UITooltip>
                  <PopoverContent align="end" className="w-64 space-y-3">
                    ...
                    <p className="text-[10px] text-muted-foreground leading-relaxed border-t pt-2">
                      90% = banda mais estreita · 95% = padrão · 99% = mais ampla. O
                      multiplicador t é recalculado para cada nível conforme os graus de
                      liberdade.
                    </p>
                    <Button
                      variant="ghost"
              ...
              {bandMode === 'see' && (
                <Popover>
                  <UITooltip>
                    ...
                    <TooltipContent side="top" className="text-xs max-w-[220px]">
                      Multiplicador z aplicado à largura da banda SEE. 1.96 ≈ 95% de
                      cobertura.
                    </TooltipContent>
                  </UITooltip>
                  <PopoverContent align="end" className="w-64 space-y-3">
        ...
        <div className="relative h-full w-full">
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              />
              <Tooltip
                content={<CustomTooltip mode={bandMode} bandLabel={bandLabel} />}
              />
