      ...
      <CardContent className="space-y-6">
        ...
        <div className="pt-4 border-t border-border/40 space-y-3">
          <div className="grid grid-cols-3 gap-2">
            ...
            <Button onClick={onTestLevelUp} size="sm" className={`gap-1.5 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white transition-all ${activeCelebration === 'levelup' ? 'animate-pulse ring-2 ring-amber-400' : ''}`} disabled={(selectedSound === 'none' && volume === 0) || (activeCelebration !== null && activeCelebration !== 'levelup') || isLoading}>
              {isLoading && activeCelebration === 'levelup' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Crown className="h-3.5 w-3.5" />} Level Up
            </Button>
            <Button onClick={onTestStreakRecord} size="sm" className={`gap-1.5 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-600 hover:to-amber-600 text-white transition-all ${activeCelebration === 'record' ? 'animate-pulse ring-2 ring-yellow-400' : ''}`} disabled={(selectedSound === 'none' && volume === 0) || (activeCelebration !== null && activeCelebration !== 'record') || isLoading}>
