...
export function RoleManagement() {
  const { allUserRoles, isLoadingAllRoles, updateRole, isUpdatingRole, canManageRoles } =
    useUserRoles();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <h4 className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-[0.2em]">
          Operational Access Control
        </h4>
        <p className="text-sm text-muted-foreground italic font-medium">
          Assign roles to manage tactical permissions across the command unit.
        </p>
      </div>
      ...
      <div className="bg-muted/10 border border-border/10 rounded-xl overflow-hidden">
                  ...
                  <TableCell>
                    {user.auth_user_id ? (
                      <Select
                        value={user.appRole}
                        onValueChange={value =>
                          handleRoleChange(user.auth_user_id!, value as AppRole)
                        }
        ...
        <div className="mt-6 p-4 bg-muted/50 rounded-lg">
          ...
          <ul className="space-y-1 text-sm text-muted-foreground">
            <li>
              <Crown className="h-4 w-4 inline mr-2 text-warning" />
              <strong>Admin:</strong> Acesso total, gerenciamento de roles, exclusão de
              dados
            </li>
            <li>
              <ShieldCheck className="h-4 w-4 inline mr-2 text-info" />
              <strong>Gerente:</strong> Visualizar relatórios, editar metas, gerenciar
              equipe
            </li>
            <li>
              <User className="h-4 w-4 inline mr-2" />
              <strong>Vendedor:</strong> Acesso às próprias vendas, atividades e metas
            </li>
          </ul>
        </div>
      </div>
