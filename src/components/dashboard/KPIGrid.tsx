...
const KPIRow = React.memo(function KPIRow({ label, value, format, icon: Icon, index }: { label: string; value: number; format: string; icon: any; index: number }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-5xl sm:text-6xl font-display font-black tabular-nums tracking-tighter z-10" style={{ textShadow: '0 0 40px hsl(var(--primary) / 0.8)' }}>{formatted}</span>
