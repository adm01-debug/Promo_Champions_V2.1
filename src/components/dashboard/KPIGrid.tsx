...
const KPIRow = React.memo(function KPIRow({ label, value, format, icon: Icon, index }: { label: string; value: number; format: string; icon: any; index: number }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-3xl sm:text-4xl font-display font-black tabular-nums tracking-tighter z-10" style={{ textShadow: '0 0 20px hsl(var(--primary) / 0.5)' }}>{formatted}</span>
