...
const getConfidenceConfig = (confidence: number) => {
  if (confidence >= 80)
    return {
      ...
      label: 'OPTIMAL',
      description: 'Consistently strong data streams detected.',
    };
  if (confidence >= 60)
    return {
      color: 'bg-warning',
      ...
      label: 'STABLE',
      description: 'Indicator variance within acceptable limits.',
    };
  return {
    color: 'bg-destructive',
    textColor: 'text-destructive',
    label: 'LOW-SIG',
    description: 'Insufficient telemetry for precise projection.',
  };
};
        ...
        <div className="space-y-4 p-5 rounded-xl bg-white/[0.03] border border-white/5 shadow-inner">
            ...
            <div className="flex items-center gap-2">
              ...
              <TooltipProvider delayDuration={200}>
                <Tooltip>
                  ...
                  <TooltipContent
                    side="top"
                    className="bg-black/90 backdrop-blur-xl border border-white/10 max-w-[220px]"
                  >
                    <p className="text-[10px] font-mono font-black uppercase mb-1 text-primary">
          ...
          <div className="relative h-2 bg-black/50 rounded-full overflow-hidden border border-white/5 p-[1px]">
            <motion.div
              ...
              style={{
                boxShadow: `0 0 10px ${config.textColor.replace('text-', '') === 'success' ? 'rgba(34, 197, 94, 0.3)' : 'rgba(234, 179, 8, 0.3)'}`,
              }}
            />
          </div>
