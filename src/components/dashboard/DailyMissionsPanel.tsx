...
import { Target, Zap, CheckCircle2, Star } from 'lucide-react';
...
export function DailyMissionsPanel() {
  const { user } = useAuth();
  const { missions, isLoading, claimXP, completedCount, totalCount } = useDailyMissions(
    user?.id
  );
          ...
          <CardHeader className="py-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge
                  variant="secondary"
                  className="bg-muted/50 text-muted-foreground font-bold"
                >
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-4 mb-2 relative z-10">
                    ...
                    <Badge
                      variant="outline"
                      className="bg-accent/10 text-accent border-accent/20 font-bold shrink-0"
                    >
                  ...
                  <div className="space-y-2 relative z-10">
                    <div className="flex justify-between text-[10px] font-semibold uppercase tracking-wider">
                      <span
                        className={
                          mission.completed ? 'text-success' : 'text-muted-foreground'
                        }
                      >
                        {mission.completed
                          ? 'Missão Cumprida!'
