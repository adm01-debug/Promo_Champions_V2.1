...
export const FuturisticSpeedometerDashboard = () => {
  ...
  } = useDashboardKPIsPeriod(period, targetSalespersonId);
  ...
  const selectedLabel =
    salespersonFilter === ALL_SALESPEOPLE
      ...
      : salespersonFilter === ME
        ? 'Meus Dados'
        : salespeople.find(s => s.id === salespersonFilter)?.name || 'Vendedor';
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Select value={salespersonFilter} onValueChange={setSalespersonFilter}>
            ...
            <SelectContent className="bg-background/95 backdrop-blur-3xl border-border/40">
              <SelectItem
                value={ALL_SALESPEOPLE}
                className="text-[10px] font-mono uppercase"
              >
                Equipe Completa
              </SelectItem>
              <SelectItem value={ME} className="text-[10px] font-mono uppercase">
                Meus Dados
              </SelectItem>
              {salespeople.map(s => (
                <SelectItem
      ...
      {kpis && (
        <div
          className={cn(
            'mt-6 relative overflow-hidden rounded-xl p-5',
            theme === 'cyber'
          ...
          <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              },
              {
                label: 'Conversão',
                curr: conversion,
                prev: kpis?.previous.conversionRate ?? 0,
                ...
                <div key={row.label} className="group relative">
                  ...
                  <div className="h-1.5 w-full bg-muted/30 rounded-full overflow-hidden relative">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${Math.min(100, (row.curr / Math.max(row.curr, row.prev, 1)) * 100)}%`,
                      }}
