...
export const Speedometer = memo(
  ({
    ...
    const range = max - min;
    const animatedPct = Math.min(
      1,
      Math.max(0, range > 0 ? (animatedValue - min) / range : 0)
    );
    ...
    const handleExportCSV = () => {
      ...
      link.setAttribute(
        'download',
        `drilldown_${label.toLowerCase()}_${drilldownPeriod}.csv`
      );
    ...
    const handleExportPDF = () => {
      ...
      doc.text(
        `Período: ${PERIOD_OPTIONS.find(p => p.value === drilldownPeriod)?.label}`,
        14,
        32
      );
      ...
      doc.text(
        `Valor Acumulado no Período: ${formatValue ? formatValue(summaryVal) : summaryVal}`,
        14,
        58
      );
    ...
    useEffect(() => {
      ...
      const multiplier =
        drilldownPeriod === 'year'
          ? 12
          : drilldownPeriod === 'quarter'
            ? 3
            : drilldownPeriod === 'last_month'
              ? 1.1
              : 1;
      ...
      >
        <div className="relative flex items-center justify-between w-full mb-3 px-2">
          <div className="flex items-center gap-2">
            <div
              className={cn(
                'p-1.5 rounded-lg bg-background/60 border border-border/40',
                colors.text
              )}
            >
            ...
            <div className="relative cursor-pointer hover:brightness-110 transition-all flex flex-col items-center">
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="relative" style={{ width: s, height: s }}>
                      ...
                      {theme === 'cyber' && !isSmallScreen && (
                        <motion.div
                          ...
                          style={{
                            ...
                            WebkitMask:
                              'radial-gradient(circle, transparent 47%, #000 49%, #000 50%, transparent 51%)',
                        ...
                        {ticks.map((t, i) => (
                          <g key={i}>
                            <line
                              ...
                              y2={t.y2}
                              stroke={
                                t.active ? colors.stroke : 'hsl(var(--muted-foreground))'
                              }
                            ...
                            {t.major && (
                              ...
                              >
                                {t.labelValue >= 1000
                                  ? Math.round(t.labelValue / 1000) + 'k'
                                  : Math.round(t.labelValue)}
                              </text>
                            )}
                          </g>
                        ))}
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        <div
                          className={cn(
                            'text-3xl font-black font-mono tracking-tighter mb-0',
                            colors.text
                          )}
                        >
                          {displayValue}
                        ...
                        <div
                          className={cn(
                            'text-[10px] font-bold font-mono uppercase tracking-widest px-2 py-0.5 rounded-full bg-background/40 border border-border/20',
                            statusColor
                          )}
                        >
                          {statusLabel}
                  ...
                  <TooltipContent
                    side="bottom"
                    className="max-w-[200px] bg-background/95 backdrop-blur-xl border-primary/20 p-3"
                  >
                    <div className="space-y-2">
                      ...
                      <p className="text-[10px] leading-relaxed text-muted-foreground">
                        {explanation || `Análise de ${label} em tempo real.`}
                      </p>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden bg-background/95 backdrop-blur-3xl border-primary/20 p-0 flex flex-col">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Select
                    ...
                    onValueChange={v => setDrilldownPeriod(v as KPIPeriod)}
                  >
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        size="icon"
                        className="h-9 w-9 bg-background/40 border-border/40"
                      >
                    ...
                    <PopoverContent
                      align="end"
                      className="w-40 p-2 bg-background/95 backdrop-blur-3xl border-border/40"
                    >
                      <div className="grid gap-1">
                        <Button
                          variant="ghost"
                          size="sm"
                          className="justify-start gap-2 text-[10px] font-mono uppercase"
            ...
            <ScrollArea className="flex-1 p-6">
              ...
              <div className="space-y-8">
                <div className="p-6 rounded-2xl bg-background/40 border border-border/40">
                  <h4 className="text-[11px] font-black font-mono uppercase tracking-[0.2em] text-muted-foreground mb-6">
                    Tendência Temporal
                  </h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-6 rounded-2xl bg-background/40 border border-border/40">
                    <h4 className="text-[11px] font-black font-mono uppercase tracking-[0.2em] text-muted-foreground mb-4">
                    ...
                    <div className="space-y-4">
                      {[
                        ...
                        <div key={i} className="flex items-center gap-4">
                          <div
                            className={cn(
                              'p-2 rounded-lg bg-background border border-border/20',
                              channel.color
                            )}
                          >
                  ...
                  <div className="p-6 rounded-2xl bg-background/40 border border-border/40">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        <div
                          key={i}
                          className="flex items-start gap-3 p-3 rounded-xl bg-background/40 border border-border/20 hover:border-primary/30 transition-colors group cursor-pointer"
                        >
            ...
            <div className="p-4 border-t border-border/40 flex justify-between items-center bg-background/60">
              ...
              <Button
                variant="ghost"
                size="sm"
                className="h-8 text-[10px] font-mono uppercase gap-2"
