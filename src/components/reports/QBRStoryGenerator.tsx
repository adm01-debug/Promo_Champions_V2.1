...
import { Sparkles, Trophy, Rocket, ShieldAlert, TrendingUp } from 'lucide-react';
...
export const QBRStoryGenerator = ({
  metrics,
  topSellers,
  periodLabel,
}: QBRStoryGeneratorProps) => {
        ...
        <div className="space-y-8 relative">
          ...
          >
            ...
            <h4 className="text-sm font-bold text-foreground">
              O Grande Capítulo: Conquistas
            </h4>
            <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
              O {periodLabel} foi marcado por um faturamento total de{' '}
              ...
              {bestSeller ? (
                <>
                  {' '}
                  O destaque absoluto foi{' '}
                  <span className="text-primary font-bold">{bestSeller.name}</span>, que
                  sozinho garantiu{' '}
                  {((bestSeller.revenue / metrics.totalRevenue) * 100).toFixed(0)}% da
                  meta batida, fechando {bestSeller.deals} deals estratégicos.
                </>
              ) : (
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
              ...
              <span className="font-bold">{metrics.conversionRate.toFixed(1)}%</span>, a
              eficiência tática foi{' '}
              {metrics.conversionRate > 25 ? 'excepcional' : 'estável'}. O ticket médio de{' '}
              <span className="font-bold text-primary">
                R$ {(metrics.avgTicket / 1000).toFixed(1)}k
              </span>{' '}
              indica que o time está focando em contas de{' '}
              {metrics.avgTicket > 50000 ? 'Enterprise' : 'Mid-market'}, mantendo o ritmo
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
              ...
              no caminho. A IA detectou que o principal "vazamento" de receita ocorre na
              etapa de Proposta Técnica. Se tivéssemos convertido 10% desses deals
              perdidos, teríamos adicionado mais R${' '}
              {(
                ((metrics.totalRevenue / (metrics.wonCount || 1)) *
                  metrics.lostCount *
                  0.1) /
                1000
              ).toFixed(1)}
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
              Para o próximo período, temos um pipeline ativo de{' '}
              <span className="text-primary font-bold">
                R$ {(metrics.pipelineValue / 1000).toFixed(1)}k
              </span>
              . O foco deve ser a <span className="font-bold">aceleração de ciclo</span>,
              já que a volumetria está saudável mas a velocidade estagnou em alguns
              estágios críticos.
            </p>
          </motion.div>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-border/50 grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <p className="text-sm font-bold mt-1">
              {isPositiveQuarter ? 'Trimestre de Expansão' : 'Trimestre de Consolidação'}
            </p>
          </div>
          <div>
            <p className="text-[10px] uppercase font-black tracking-widest text-muted-foreground">
