...
export function ActivityLogForm({
  ...
  const templates: Record<ActivityType, { label: string; text: string }[]> = {
    call: [
      {
        label: 'Caixa Postal',
        text: 'Deixado recado na caixa postal. Agendado novo follow-up.',
      },
      { label: 'Qualificação BANT', text: 'Budget: \nAuthority: \nNeed: \nTimeline: ' },
      {
        label: 'Conexão Sucedida',
        text: 'Conversamos sobre [DOR]. Demonstrou interesse em [PRODUTO].',
      },
    ],
    email: [
      {
        label: 'Follow-up #1',
        text: 'Olá [NOME], estou acompanhando nosso último contato sobre [ASSUNTO]...',
      },
      {
        label: 'Cold Outreach',
        text: 'Vi que você atua com [SETOR] e gostaria de compartilhar como ajudamos...',
      },
    ],
    linkedin: [
      {
        label: 'Pedido Conexão',
        text: 'Olá [NOME], acompanho seu trabalho em [EMPRESA] e gostaria de conectar.',
      },
      {
        label: 'Mensagem InMail',
        text: 'Notei seu interesse em [ASSUNTO] e acredito que podemos colaborar...',
      },
    ],
    whatsapp: [
      {
        label: 'Confirmar Reunião',
        text: 'Oi [NOME], passando para confirmar nossa reunião hoje às [HORA]. Podemos manter?',
      },
      {
        label: 'Follow-up Rápido',
        text: 'Conseguiu dar uma olhada no material que te enviei por e-mail?',
      },
    ],
    meeting: [
      {
    ...
    note: [
      {
        label: 'Insight ICP',
        text: 'Cliente se encaixa perfeitamente no perfil de [SEGMENTO] devido a [RAZÃO].',
      },
    ],
    other: [],
  };
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-4 lg:grid-cols-7 gap-2">
                    {activityTypes.map(type => {
                      ...
                      return (
                        ...
                        >
                          <Icon
                            className={`h-4 w-4 ${field.value === type.value ? 'gradient-primary' : ''}`}
                          />
                          {type.label}
                        </button>
                      );
                  ...
                  <Popover open={salespersonOpen} onOpenChange={setSalespersonOpen}>
                    ...
                    >
                      <Command className="glass border-border/50">
                        ...
                        />
                        <CommandList>
                          <CommandEmpty className="text-xs py-2 px-4">
