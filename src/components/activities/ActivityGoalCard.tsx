...
import { Phone, Mail, Calendar, MessageCircle, Linkedin, Settings } from "lucide-react";
...
export function ActivityGoalCard({ data, onEdit }: ActivityGoalCardProps) {
  ...
  const getStatusBadge = () => {
    if (data.progress.overall >= 100) {
      return <Badge className="bg-green-500/20 text-green-400 text-[10px]">🎯 Meta Batida!</Badge>;
    }
  ...
  return (
    <Card className="glass border-border/40 hover:border-border/60 transition-all">
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-medium text-sm">{data.salesperson_name}</span>
          ...
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onEdit(data.salesperson_id)}>
          ...
          <>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-sm font-bold">{data.progress.overall.toFixed(0)}%</span>
              </div>
              <div className="h-2 rounded-full bg-muted overflow-hidden">
                <div 
                  className={`h-full rounded-full transition-all ${getProgressColor(data.progress.overall)}`}
