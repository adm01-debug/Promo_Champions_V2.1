...
import { Activity, Zap, Flame, Radar, Ghost, Crown, Medal } from 'lucide-react';
...
export const ArenaPulseFeed: React.FC<ArenaPulseFeedProps> = ({ data }) => {
  ...
  return (
    <Card
      variant="glass"
      className="overflow-hidden border-primary/20 shadow-2xl group/feed relative"
    >
      ...
      <CardHeader className="pb-3 border-b border-white/10 bg-gradient-to-r from-primary/10 via-transparent to-transparent relative">
        <div className="flex flex-col gap-1.5">
          <CardTitle className="text-xs font-black uppercase tracking-[0.3em] flex items-center justify-between">
            ...
            <Badge
              variant="outline"
              className="bg-primary/5 text-[8px] font-black border-primary/30 animate-pulse"
            >
              SISTEMA ATIVO
            </Badge>
          </CardTitle>
          <div className="flex items-center gap-2 px-1">
            <div className="flex -space-x-1.5 overflow-hidden">
              {data.slice(0, 3).map((sp, i) => (
                <div
                  key={i}
                  className="w-4 h-4 rounded-full border border-background bg-muted text-[6px] font-black flex items-center justify-center uppercase"
                >
                ...
                >
                  ...
                  <div className="flex-1 space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[8px] font-bold text-muted-foreground/40 font-mono">
                          {event.time}
                        </span>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {event.type === 'victory' && (
                        <>
                          ...
                          <Badge
                            variant="outline"
                            className="text-[7px] border-primary/30 text-primary bg-primary/10 font-black h-4 px-2 py-0"
                          >
                            +500 XP Gained
                          </Badge>
                        </>
                      ...
                      {event.type === 'social' && (
                        <div className="flex gap-1">
                          {['🔥', '⚡', '👏'].map((emoji, i) => (
                            <button
                              key={i}
                              className="w-5 h-5 rounded-md bg-white/5 border border-white/10 flex items-center justify-center text-[10px] hover:bg-primary/20 transition-colors"
                            >
                              {emoji}
                            </button>
                          ))}
