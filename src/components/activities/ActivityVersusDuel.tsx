...
export const ActivityVersusDuel: React.FC<ActivityVersusDuelProps> = ({ data }) => {
  ...
  return (
    <Card
      variant="glass"
      className="overflow-hidden border-accent/20 bg-accent/5 backdrop-blur-xl relative"
    >
      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between gap-4 relative">
            ...
            <div className="relative inline-block group">
              ...
              <Avatar className="h-20 w-20 border-2 border-primary/50 relative shadow-glow-primary/20 transition-transform group-hover:scale-110">
                ...
                <AvatarFallback className="bg-primary/20 text-primary font-black text-xl">
                  {d1.salesperson_name[0]}
                </AvatarFallback>
              </Avatar>
              <div className="absolute -bottom-1 -right-1 bg-primary text-[10px] font-black px-2.5 py-1 rounded-md text-white shadow-glow-primary/40 border border-white/20 transform rotate-3">
                1º PLACE
              </div>
            </div>
            <div>
              <p className="text-sm font-black truncate tracking-tighter uppercase italic">
                {d1.salesperson_name.split(' ')[0]}
              </p>
              <p className="text-lg font-display font-black text-primary drop-shadow-glow">
                {d1.progress.overall.toFixed(0)}%
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center relative px-4">
            <div className="text-3xl font-display font-black italic text-muted-foreground/20 leading-none select-none">
              VS
            </div>
          ...
          <div className="flex-1 text-center space-y-4">
            <div className="relative inline-block group">
              ...
              <Avatar className="h-20 w-20 border-2 border-accent/50 relative shadow-glow-accent/20 transition-transform group-hover:scale-110">
                ...
                <AvatarFallback className="bg-accent/20 text-accent font-black text-xl">
                  {d2.salesperson_name[0]}
                </AvatarFallback>
              </Avatar>
              <div className="absolute -bottom-1 -right-1 bg-accent text-[10px] font-black px-2.5 py-1 rounded-md text-white shadow-glow-accent/40 border border-white/20 transform -rotate-3">
                2º PLACE
              </div>
            </div>
            <div>
              <p className="text-sm font-black truncate tracking-tighter uppercase italic">
                {d2.salesperson_name.split(' ')[0]}
              </p>
              <p className="text-lg font-display font-black text-accent drop-shadow-glow">
                {d2.progress.overall.toFixed(0)}%
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 space-y-4">
          <div className="flex justify-between text-[11px] font-black uppercase tracking-widest px-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-accent font-black">
                {(d1.progress.overall - d2.progress.overall).toFixed(1)}%
              </span>
            </div>
          </div>
          <div className="h-2 w-full bg-muted/30 rounded-full overflow-hidden flex shadow-inner border border-white/5">
            <div
              ...
              style={{
                width: `${(d1.progress.overall / (d1.progress.overall + d2.progress.overall)) * 100}%`,
              }}
            >
            ...
            <div
              ...
              style={{
                width: `${(d2.progress.overall / (d1.progress.overall + d2.progress.overall)) * 100}%`,
              }}
            >
          ...
          <div className="p-3 rounded-xl bg-white/5 border border-white/10 flex items-center gap-3 group-hover:bg-white/10 transition-colors">
            ...
            <div className="space-y-0.5">
              <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                Projeção Arena Predictor
              </p>
              <p className="text-[10px] font-black text-foreground">
                <span className="text-accent">{d2.salesperson_name.split(' ')[0]}</span>{' '}
                precisa de{' '}
                <span className="text-primary">
                  +{((d1.progress.overall - d2.progress.overall) * 1.5).toFixed(0)} calls
                </span>{' '}
                para virar o jogo nas próximas 2h.
              </p>
            </div>
          </div>
        </div>
