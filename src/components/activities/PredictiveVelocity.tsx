...
import { Timer, TrendingUp, Clock, Zap } from 'lucide-react';
    ...
    >
      ...
      <CardContent className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[10px] text-muted-foreground font-black italic">
                / HR
              </span>
            </div>
          </div>
          <div className="text-right">
            ...
            <p
              className={cn(
                'text-2xl font-display font-black drop-shadow-glow',
                isOnTrack ? 'text-status-success' : 'text-status-warning'
              )}
            >
        ...
        <div className="p-4 rounded-2xl bg-background/50 border border-white/5 space-y-3 relative overflow-hidden group/guideline">
          ...
          <div className="flex items-center justify-between relative z-10">
            ...
            <Badge
              variant="outline"
              className={cn(
                'text-[10px] font-black border-none px-2 py-0.5 rounded-md',
                isOnTrack
                  ? 'bg-status-success/20 text-status-success'
                  : 'bg-status-warning/20 text-status-warning animate-pulse'
              )}
            >
          ...
          <div className="space-y-2 pt-2 relative z-10">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div
                  className={cn(
                    'w-2 h-2 rounded-full animate-pulse shadow-glow-primary',
                    isOnTrack ? 'bg-status-success' : 'bg-status-warning'
                  )}
                />
        ...
        <div className="flex items-center justify-between px-1">
          <span className="text-[8px] font-black uppercase tracking-tighter text-muted-foreground/50">
            Confiança do Sistema Predictor
          </span>
            ...
            {[1, 2, 3, 4, 5].map(i => (
              <div
                ...
                className={cn(
                  ...
                  i <= 4 ? 'bg-primary shadow-glow-primary' : 'bg-white/10'
                )}
              />
            ))}
          </div>
