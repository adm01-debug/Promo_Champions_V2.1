          ...
          <div className="mt-2 grid grid-cols-2 gap-2 p-2 rounded bg-black/20 border border-white/5">
            <div className="text-center">
              ...
              <p className="text-xs font-black text-primary">
                #{String(n.metadata.seller_rank || '0')}
              </p>
            </div>
            <div className="text-center border-l border-white/10">
              ...
              <p className="text-xs font-black text-foreground">
                #{String(n.metadata.recipient_rank || '0')}
              </p>
            </div>
          </div>
        ) : null}
...
export function NotificationCenter() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'ranking')
      return notifications.filter(n => n.category === 'gamification');
    if (filter === 'sales')
      return notifications.filter(n => n.category === 'sales' || n.type === 'sale_alert');
                ...
                <div className="space-y-6 py-4">
                  <div className="flex items-center justify-between p-4 rounded-lg bg-black/20 border border-white/5">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Receba popups e notificações em tempo real no HUD.
                      </p>
                    </div>
                    <Switch
                  ...
                  <div className="flex items-center justify-between p-4 rounded-lg bg-black/20 border border-white/5">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Receba um resumo de cada venda fechada no seu e-mail.
                      </p>
                    </div>
                    <Switch
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 glass">
                ...
                <DropdownMenuItem
                  ...
                  className={cn(filter === 'all' && 'bg-primary/10 text-primary')}
                >
                  Todas
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => setFilter('unread')}
                  className={cn(filter === 'unread' && 'bg-primary/10 text-primary')}
                >
                  Não lidas
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => setFilter('sales')}
                  className={cn(filter === 'sales' && 'bg-primary/10 text-primary')}
                >
                  <TrendingUp className="h-3.5 w-3.5 mr-2" /> Vendas
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => setFilter('ranking')}
                  className={cn(filter === 'ranking' && 'bg-primary/10 text-primary')}
                >
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center opacity-40">
              ...
              <p className="text-[10px] mt-1 uppercase">
                Novos alertas aparecerão em tempo real
              </p>
            </div>
          ) : (
