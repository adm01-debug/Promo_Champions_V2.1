        ...
        <SidebarMenuButton asChild tooltip={item.title}>
          ...
          >
            ...
            <div className="relative">
              <item.icon className={cn("h-5 w-5 flex-shrink-0 transition-transform duration-200 group-hover/item:scale-110", hasAlerts && "text-warning")} />
              {hasAlerts && (
  ...
  const renderGroupedMenu = (group: MenuGroup) => {
    ...
    return (
      <Collapsible key={group.label} defaultOpen={hasActiveChild} className="group/collapsible">
        <CollapsibleTrigger className="flex items-center gap-2.5 w-full px-3 py-2 rounded-lg text-muted-foreground/60 hover:text-muted-foreground hover:bg-muted/20 transition-all duration-200 text-[10px] uppercase tracking-[0.1em] font-bold">
          <div className="p-1 rounded-md bg-muted/40 group-data-[state=open]/collapsible:bg-primary/10">
            <group.icon className="h-3 w-3 flex-shrink-0 group-data-[state=open]/collapsible:text-primary transition-colors" />
          </div>
          {!isCollapsed && (
            <>
              <span className="flex-1 text-left group-data-[state=open]/collapsible:text-foreground/70 transition-colors">{group.label}</span>
        ...
        <CollapsibleContent className="overflow-hidden data-[state=open]:animate-collapsible-down data-[state=closed]:animate-collapsible-up">
          <SidebarMenu className="space-y-0.5 pl-2 mt-0.5 ml-[13px] border-l border-border/20">
    ...
    <Sidebar collapsible="icon" className="border-r border-border/30 bg-sidebar/95 backdrop-blur-sm">
      ...
      {isAdminOrManager && !isCollapsed && (
        <div className="px-3 pb-3">
          <div className="flex gap-1 p-1 bg-sidebar-background/80 rounded-xl border border-sidebar-border/50">
            {viewModes.map((vm) => {
              const isActive = viewMode === vm.mode;
              return (
                <Button key={vm.mode} variant="ghost" size="sm" onClick={() => setViewMode(vm.mode)}
                  className={cn("flex-1 h-8 text-[11px] font-semibold rounded-lg transition-all duration-300 gap-1.5 px-2",
                    isActive 
                      ? "bg-gradient-to-r from-primary to-primary/90 text-primary-foreground shadow-md shadow-primary/20 hover:from-primary/90 hover:to-primary/80" 
