...
export const CadenceStepRow = React.memo(function CadenceStepRow({
  step,
  onSave,
  onDelete,
  isSaving,
  ...
  const Icon =
    ACTION_TYPES.find(a => a.value === step.action_type)?.icon || MoreHorizontal;
  ...
  const handleSave = () => {
    ...
    if (
      currentTaskType === 'automatic' &&
      ['email', 'whatsapp', 'linkedin'].includes(currentActionType)
    ) {
      ...
      <div className="p-3 rounded-lg glass border border-border/30 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={e =>
                setEditData(d => ({ ...d, day_number: parseInt(e.target.value) || 1 }))
              }
          ...
          <div className="space-y-1">
            ...
            <Select
              ...
              onValueChange={v =>
                setEditData(d => ({ ...d, action_type: v as ActionType }))
              }
            >
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="flex items-center justify-between p-2 rounded bg-background/40 border border-border/40">
            ...
            <input
              ...
              onChange={e =>
                setEditData(d => ({ ...d, needs_approval: e.target.checked }))
              }
            />
          </div>
          <div className="flex items-center justify-between p-2 rounded bg-background/40 border border-border/40">
        ...
        <div className="space-y-1">
            ...
            <MergeTagPicker
              ...
              onInsert={token =>
                setEditData(d => ({
                  ...d,
                  template_content:
                    (d.template_content ?? step.template_content ?? '') + token,
                }))
              }
            />
          </div>
          ...
          <p className="text-[10px] text-muted-foreground">
            Use variáveis como {`{{cliente.nome}}`}, {`{{negocio.valor}}`}
          </p>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {step.needs_approval && (
              <Badge
                variant="outline"
                className="text-[8px] h-4 bg-status-warning/10 text-status-warning border-status-warning/20"
              >
                Aprovação
              </Badge>
            )}
