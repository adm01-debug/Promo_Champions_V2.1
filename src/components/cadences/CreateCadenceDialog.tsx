...
const stepInputSchema = z
  ...
  .refine(
    data => {
      if (
        data.task_type === 'automatic' &&
        ['email', 'whatsapp', 'linkedin'].includes(data.action_type)
      ) {
...
export function CreateCadenceDialog() {
  ...
  const updateStep = <K extends keyof StepInput>(
    index: number,
    field: K,
    value: StepInput[K]
  ) => {
    ...
    >
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto glass border-border/50 dark:border-glow">
        ...
        {step === 'info' ? (
          <Form {...form}>
            <form
              onSubmit={form.handleSubmit(handleCreateCadence)}
            ...
            >
              ...
              >
                {createCadence.isPending ? 'Criando...' : 'Continuar'}
              </Button>
            </form>
          </Form>
        ) : (
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              {steps.map((s, index) => (
                <div
                  key={index}
                  className={`p-4 rounded-lg border bg-muted/30 hover:bg-muted/40 space-y-3 transition-colors ${stepErrors[index] ? 'border-destructive/50 ring-1 ring-destructive/20' : 'border-border/50'}`}
                >
                  <div className="flex items-center justify-between">
                    <Badge
                      variant="secondary"
                      className="text-xs bg-gradient-to-r from-primary/20 to-accent/10 text-primary border border-primary/20"
                    >
                      Etapa {index + 1}
                    </Badge>
                    {steps.length > 1 && (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        value={s.day_number}
                        onChange={e =>
                          updateStep(index, 'day_number', parseInt(e.target.value) || 1)
                        }
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex items-center justify-between p-2 rounded bg-background/40 border border-border/40">
                      ...
                      <input
                        ...
                        onChange={e =>
                          updateStep(index, 'needs_approval', e.target.checked)
                        }
                      />
                    </div>
                    <div className="flex items-center justify-between p-2 rounded bg-background/40 border border-border/40">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea
                      ...
                      onChange={e =>
                        updateStep(index, 'template_content', e.target.value)
                      }
                      placeholder="Conteúdo..."
                      className={`min-h-[80px] text-sm resize-none bg-background/50 font-mono ${stepErrors[index]?.some(e => e.toLowerCase().includes('template')) ? 'border-destructive/50' : ''}`}
                    />
                  </div>
                  {stepErrors[index] && (
            ...
            <Button
              variant="outline"
              className="w-full gap-2 border-dashed border-border/60"
              onClick={addStep}
            >
              <Plus className="h-4 w-4" />
              Adicionar Etapa
