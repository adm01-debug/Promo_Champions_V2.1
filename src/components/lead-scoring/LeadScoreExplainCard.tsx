...
export const LeadScoreExplainCard = React.memo(
  ({ saleId, churnRisk, onActionComplete }: Props) => {
    ...
    const steps = [
      ...
      },
      {
        ...
        description:
          'Nesta etapa, a rede neural analisa micro-interações: frequência de e-mails, aberturas de propostas e velocidade de resposta. Cada driver abaixo é um neurônio ativo que soma ou subtrai força do score final.',
        icon: Activity,
        color: 'text-primary',
        impact: exp.top_drivers
      ...
      },
      {
        ...
        description:
          churnRisk && churnRisk.risk_score > 30
        ...
        icon: ShieldAlert,
        color:
          churnRisk && churnRisk.risk_score > 50 ? 'text-rose-500' : 'text-emerald-500',
          ...
          <Card className="bg-status-error/5 border-status-error/20 overflow-hidden relative shadow-[0_0_20px_rgba(239,68,68,0.1)]">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xl font-black text-status-error italic">
                  {churnRisk.risk_score}%
                </span>
              </div>
              <Progress
                value={churnRisk.risk_score}
                className="h-2 bg-status-error/10"
              ...
              <div className="pt-2">
                <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-2 flex items-center gap-1.5">
                  <Brain className="h-3 w-3 text-status-error" /> Principais Fatores de
                  Risco:
                </p>
                <ul className="grid grid-cols-1 gap-1.5">
                  {churnRisk.factors.map((f, i) => (
                    <li
                      key={i}
                      className="text-[10px] text-muted-foreground bg-background/50 p-2 rounded border border-status-error/10 flex items-start gap-2 italic"
                    >
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 px-1">
            ...
            <h4 className="text-xs font-black uppercase tracking-widest text-muted-foreground">
              Neural Matrix: Decomposição de Score
            </h4>
          </div>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isExpanded
                      ? 'bg-primary/5 border-primary/20 shadow-[0_0_15px_rgba(var(--primary-rgb),0.05)]'
                      : 'bg-card/40 border-white/5 hover:bg-primary/5'
                  )}
                >
                  <div
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={cn(
                          'p-2.5 rounded-xl bg-background/50 ring-1 ring-inset ring-white/10 shadow-inner group-hover:scale-110 transition-transform',
                          step.color
                        )}
                      >
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h5 className="text-[12px] font-black uppercase tracking-widest">
                            {step.title}
                          </h5>
                          <Badge
                            variant="outline"
                            className="text-[7px] font-black tracking-[0.2em] border-white/5 opacity-50 group-hover:opacity-100 transition-opacity"
                          >
                            {step.badge}
                          </Badge>
                        </div>
                        <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-tighter mt-0.5">
                          ...
                          <span
                            className={cn(
                              'font-bold',
                              step.impact >= 0 ? 'text-emerald-500' : 'text-rose-500'
                            )}
                          >
                            {step.impact >= 0 ? '+' : ''}
                            {step.impact} pts
                      ...
                      {idx === 1 && (
                        <div className="grid grid-cols-1 gap-3">
                          {exp.top_drivers.map(d => (
                            <div
                              key={d.factor}
                              className="space-y-1.5 p-3 rounded-lg bg-background/30 border border-white/5 group/driver hover:bg-background/50 transition-colors"
                            >
                              ...
                              <div className="flex items-center gap-1.5 opacity-0 group-hover/driver:opacity-100 transition-opacity">
                                ...
                                <span className="text-[8px] font-black text-primary uppercase tracking-widest">
                                  IA Insight: Driver Crítico para Conversão
                                </span>
                              </div>
                            </div>
                          ))}
                        ...
                        <div className="mt-2 p-3 rounded-lg bg-status-error/5 border border-status-error/10">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[9px] font-black text-status-error uppercase tracking-widest">
                              Ajuste Neural por Risco de Churn
                            </span>
                          </div>
                          <p className="text-[10px] text-muted-foreground font-medium leading-relaxed italic">
                            O score final foi penalizado em{' '}
                            <span className="text-status-error font-bold">
                              {Math.round(churnRisk.risk_score / 2)} pontos
                            </span>{' '}
                            devido a sinais críticos de instabilidade na conta.
                          </p>
                        </div>
                      )}
                    </div>
          ...
          <div className="md:col-span-5">
            <Card className="h-full glass border-white/5 bg-gradient-to-br from-card/80 to-card/40 flex flex-col items-center justify-center p-6 relative overflow-hidden group">
              ...
              <div className="relative mb-4">
                ...
                <span
                  className={cn(
                    'text-7xl font-display font-black tracking-tighter italic z-10',
                    scoreColor
                  )}
                >
                  {exp.score}
              ...
              <p className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest mb-4 z-10">
                Intelligence Index
              </p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                {exp.recommendations.map((r, i) => (
                  <Card
                    key={i}
                    className="group relative overflow-hidden hover:border-primary/40 transition-all duration-300 bg-card/40 border-white/5"
                  >
                    <CardContent className="p-4 flex flex-col">
                      ...
                      <p className="text-xs font-bold leading-relaxed mb-4">{r.action}</p>
