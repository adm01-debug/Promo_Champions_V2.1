...
import { Crown, Medal, Award, Flame, Zap, Swords, Trophy } from 'lucide-react';
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0 relative z-10">
          <div className="flex items-center justify-between mb-1">
            <p className="font-black text-sm truncate group-hover:text-primary transition-colors tracking-tight uppercase italic flex items-center gap-2">
              {name}
              {goalProgress >= 100 && (
                <Flame className="h-3 w-3 text-streak animate-pulse" />
              )}
            </p>
            <div className="flex items-center gap-1">
              <Zap className="h-3 w-3 text-primary animate-pulse" />
              <span className="text-[10px] font-black text-primary">
                XP {Math.floor(totalSales / 100)}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mb-2">
            {role && (
              <Badge
                variant="outline"
                className={cn(
                  'text-[9px] font-black uppercase tracking-widest px-1.5 h-4 border-none',
                  role === 'sdr'
            ...
            <div className="flex items-center gap-3 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
              <span className="flex items-center gap-1">
                <Trophy className="h-2.5 w-2.5" /> {completedSales} sales
              </span>
              <span className="flex items-center gap-1">
                <Swords className="h-2.5 w-2.5" /> 14 duels
              </span>
            </div>
          </div>
        ...
        <div className="flex flex-col items-end gap-1 shrink-0 ml-2 relative z-10">
          {goalProgress >= 100 ? (
            <Badge className="bg-success/20 text-success border-success/30 text-[9px] font-black uppercase py-0 h-5">
              MET BATI
            </Badge>
          ) : (
