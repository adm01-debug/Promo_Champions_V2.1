...
BEGIN
  ...
  IF NOT v_is_admin THEN
    SELECT id INTO v_caller_sp
     ...
     WHERE auth_user_id = v_caller AND is_active = true
     LIMIT 1;
  ...
  -- Cria a venda (esquema real: product_name, amount)
  INSERT INTO public.sales (
    client_name, product_name, amount, salesperson_id, status, source, client_id
  ) VALUES (
    v_quote.client_name,
    LEFT(COALESCE(v_product_desc, v_quote.title), 500),
    v_quote.total_value,
    v_quote.created_by,
    'confirmed',
    'quote_conversion',
  ...
  LOOP
    ...
    BEGIN
      INSERT INTO public.orders (
        ...
        jsonb_build_object(
          'source','quote_conversion','sale_id',v_sale_id,
  ...
  RETURN jsonb_build_object(
    'sale_id', v_sale_id, 'order_id', v_order_id, 'order_number', v_order_number,
    'items_count', v_item_count, 'idempotent', false
  );
END;
$function$;
