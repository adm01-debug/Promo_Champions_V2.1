...
-- Assegura unicidade adicional (defesa em profundidade). O índice já existe;
-- rodamos IF NOT EXISTS por segurança em ambientes onde tenha sido removido.
CREATE UNIQUE INDEX IF NOT EXISTS ux_orders_quote_id
  ON public.orders (quote_id) WHERE quote_id IS NOT NULL;

DROP FUNCTION IF EXISTS public.fn_convert_quote_to_sale(uuid);
...
BEGIN
  ...
  -- 3) Idempotência: já convertido → devolve a venda existente + pedido (se houver)
  ...
  -- 7) Consistência valor x itens (só quando há itens registrados)
  ...
  IF v_item_count > 0 AND ABS(v_items_total - v_quote.total_value) > 0.02 THEN
    RAISE EXCEPTION
  ...
  -- 8) Descrição consolidada do produto para sales.product_name
  SELECT COALESCE(
  ...
  -- 10) Cria o pedido (order_number único com retry curto)
  LOOP
    v_attempts := v_attempts + 1;
    v_order_number := 'ORC-' || to_char(now(), 'YYYYMMDD') || '-'
                      || upper(substring(md5(random()::text || _quote_id::text || v_attempts::text) from 1 for 6));
    BEGIN
...
'Converte um orçamento em venda + pedido (orders/order_items) de forma transacional e idempotente. Códigos de erro: NOT_AUTHENTICATED, QUOTE_NOT_FOUND, FORBIDDEN, INVALID_STATUS, INVALID_TOTAL, TOTAL_MISMATCH.';
