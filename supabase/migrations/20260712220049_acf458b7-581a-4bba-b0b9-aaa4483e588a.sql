...
-- P1: Retenção automática de telemetria (90d) + hook para partitioning futuro.
-- Substitui pg_partman (indisponível) por purge diária idempotente. Quando qualquer
-- destas tabelas exceder ~10M rows a conversão para RANGE partition passa a valer;
-- por ora, purge basta para evitar bloat e manter autovacuum saudável (G4).
...
BEGIN
  ...
  INSERT INTO public.maintenance_log(job_name, operation, status, details)
  VALUES ('purge_old_telemetry', 'delete', 'success',
          jsonb_build_object('retention_days', _retention_days, 'cutoff', cutoff));
END;
$$;
...
COMMENT ON FUNCTION public.purge_old_telemetry(integer) IS
  'P1: retenção automática de telemetria. Roda diariamente via pg_cron; substitui pg_partman.';
