-- Publica cinco jobs operacionais sem expor credenciais nos comandos do pg_cron.
-- O allowlist impede que a função SECURITY DEFINER se torne um proxy arbitrário.
...
BEGIN
  ...
  WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'notify-v4-quote-status-every-5min'
  );
  ...
  WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'check-v4-callback-alerts-every-5min'
  );
  PERFORM cron.schedule(
  ...
  WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'cron-failure-alerter-10min'
  );
  PERFORM cron.schedule(
  ...
  WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'process-call-recording-ingest-1min'
  );
  PERFORM cron.schedule(
  ...
  PERFORM cron.unschedule('edge-retry-threshold-alert-15min')
  WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'edge-retry-threshold-alert-15min'
  );
  PERFORM cron.schedule(
    'edge-retry-threshold-alert-15min',
    '*/15 * * * *',
    'SELECT public.trigger_internal_edge_job(''edge-retry-threshold-alert'');'
  );
END;
$$;
