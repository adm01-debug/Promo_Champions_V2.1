...
Deno.serve(withRequestId('sales-assistant-chat', async (req, _ctx) => {
  ...
  try {
    ...
    if (salespersonId) {
      ...
      const pendingDeals =
        sales?.filter(s => s.status !== 'completed' && s.status !== 'cancelled') || [];
      const goalAmount = goals?.goal_amount || 0;
      const progressPercent = goalAmount
        ? Math.round((totalSales / goalAmount) * 100)
        : 0;
      ...
      const scheduledMeetings =
        activities?.filter(a => a.outcome === 'scheduled').length || 0;
...
- 🔍 Principal motivo de perda: "${topLossReason}". Prepare argumentos para essa objeção.`;
      ...
      if (activityGoals) {
        const callProgress = activityGoals.calls_goal
          ? (activityCounts.call / activityGoals.calls_goal) * 100
          : 100;
...
- 🔥 Você tem ${hotDeals.length} lead(s) quente(s). Priorize o follow-up deles!`;
      ...
      const dealsListStr = pendingDeals
        .slice(0, 10)
        .map(d => {
...
${
  outcomes
    ...
    .slice(0, 3)
    .map(o => `- Motivo de perda: ${o.reason}`)
    .join('\n') || ''
}
...
${
  objections
    ?.slice(0, 5)
    .map(o => `- "${o.objection}": ${o.response.substring(0, 100)}...`)
