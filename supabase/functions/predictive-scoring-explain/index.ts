...
function buildRecommendations(drivers: Driver[], score: number): Recommendation[] {
  ...
  for (const d of negatives) {
    if (d.factor === 'timeInPipeline') {
      ...
      recs.push({
        action:
          'Registrar nova interação (ligação, e-mail ou reunião) nos próximos 2 dias.',
        expected_lift: 7,
        priority: 'high',
      });
      ...
      recs.push({
        action:
          'Avançar o deal para a próxima etapa do funil com checklist de qualificação.',
        expected_lift: 10,
        priority: 'high',
      });
...
async function explainOne(
  ...
  if (lsErr || !ls)
    return { sale_id: saleId, score: 0, ok: false, error: 'lead_score not found' };
  ...
  if (sale?.salesperson_id) {
    ...
    if (portfolio && portfolio.length > 0) {
      baseline =
        portfolio.reduce(
          (s: number, r: { score: number | null }) => s + (r.score || 0),
          0
        ) / portfolio.length;
    }
  }
  ...
  const factors = (ls.factors || {}) as Record<string, unknown>;
  ...
  const { error: upErr } = await supabase.from('lead_score_explanations').upsert(
    {
      sale_id: saleId,
      score: ls.score,
