...
interface CollectedData extends ToolMeta {
...
const TOOLS = [
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          group_by: {
            type: 'string',
            enum: ['day', 'week', 'month', 'salesperson', 'category'],
          },
          filters: {
            type: 'object',
            properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Snapshot of current open pipeline grouped by stage with count and value.',
      parameters: {
        type: 'object',
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Aggregate completed activities (calls/emails/meetings/whatsapp) for a period.',
      parameters: {
        type: 'object',
        properties: {
          type: { type: 'string' },
...
async function resolveTool(name: string, args: ResolverArgs, supabase: SupabaseClient) {
  switch (name) {
    case 'query_sales_metric':
      return await querySalesMetric(supabase, args);
    case 'query_pipeline_snapshot':
      return await queryPipelineSnapshot(supabase, args);
    case 'query_activities':
      return await queryActivities(supabase, args);
    case 'query_top_clients':
      return await queryTopClients(supabase, args);
    default:
  ...
  try {
    ...
    if (!toolCalls.length) {
      ...
      return new Response(
        JSON.stringify({
          answer:
            choice?.content ?? 'Não consegui interpretar a pergunta. Tente reformular.',
          data: [],
          tool_calls: [],
          period: null,
        }),
    ...
    const resolvedToolMessages: ChatMessage[] = [];
    const collectedData: CollectedData[] = [];
    const collectedToolMeta: ToolMeta[] = [];

    for (const tc of toolCalls) {
      const fnName = tc.function?.name;
      let parsedArgs: ResolverArgs = {};
      try {
    ...
    return new Response(
      JSON.stringify({
        ...
        period: collectedToolMeta[0]?.args?.period_start
          ? {
              start: collectedToolMeta[0].args.period_start,
              end: collectedToolMeta[0].args.period_end,
            }
          : null,
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
