...
/** Arguments passed from the LLM tool call. All fields optional/untrusted. */
...
/** Subset of a `sales` row selected by these resolvers. */
interface SalesRow {
  id?: string;
  amount?: number | null;
  status?: string | null;
...
/** Subset of an `activities` row selected by these resolvers. */
interface ActivityRow {
  activity_type?: string | null;
}

/** Row from `salespeople_public`. */
...
export async function querySalesMetric(
  ...
  if (filters?.status && SAFE_STATUSES.has(filters.status))
    q = q.eq('status', filters.status);
  if (filters?.category && SAFE_CATEGORIES.has(filters.category))
  ...
  if (
    !filters?.status &&
    (metric === 'revenue' || metric === 'avg_ticket' || metric === 'won_count')
  ) {
  ...
  // Compute aggregates
  const totalAmount = sales.reduce((s, r: SalesRow) => s + Number(r.amount || 0), 0);
  ...
  if (metric === 'conversion_rate') {
    ...
    const won = (all ?? []).filter((r: { status?: string | null }) =>
      ['won', 'completed', 'closed'].includes(r.status ?? '')
    ).length;
  ...
  if (group_by === 'day' || group_by === 'week' || group_by === 'month') {
    ...
    for (const r of sales as SalesRow[]) {
    ...
    rows = [...buckets.entries()]
      ...
      .map(([period, v]) => ({
        ...
        revenue: v.revenue,
        count: v.count,
        avg_ticket: v.count ? v.revenue / v.count : 0,
      }));
    ...
    rows = [...buckets.entries()].map(([category, v]) => ({
      category,
      revenue: v.revenue,
      count: v.count,
    }));
...
export async function queryPipelineSnapshot(
  supabase: SupabaseClient,
  args: ResolverArgs
): Promise<ResolverResult> {
  const stage =
    typeof args?.stage === 'string' && SAFE_STATUSES.has(args.stage) ? args.stage : null;
  let q = supabase
    .from('sales')
  ...
  for (const r of (data ?? []) as SalesRow[]) {
  ...
  const total = rows.reduce((s, r: { value: number }) => s + r.value, 0);
...
export async function queryActivities(
  ...
  const rows = [...byType.entries()].map(([activity_type, count]) => ({
    activity_type,
    count,
  }));
  return {
