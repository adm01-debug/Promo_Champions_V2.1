...
Deno.test('lead-scoring contract validation - invalid payload', async () => {
  const _req = createMockRequest({ dealIds: 'not-an-array' });
  // We can't easily import the serve handler directly if it's top-level serve()
  // But we can test the validator used by it
  const { validateWebhookPayload, WebhookContracts } =
    await import('../_shared/webhook-validator.ts');

  const result = validateWebhookPayload(
    WebhookContracts.leadScoring,
    { dealIds: 'not-an-array' },
    '1.0.0'
  );
