...
async function loadContext(
  ...
  const [{ data: client }, { data: activities }] = await Promise.all([
    sale.client_id
      ? admin
          .from('clients')
          .select('id, name, email, phone, company_name, segment, notes')
            ...
            parameters: {
              ...
              properties: {
                ...
                personalization_notes: {
                  type: 'string',
                  description: '1 frase explicando o gancho usado',
                },
              },
...
function fallback(ctx: EmailContext, prompt: string): EmailDraft {
  const name = (ctx?.client?.name as string | undefined) ?? 'olá';
  ...
  try {
    ...
    const rows = generations
      ...
      .map(g => ({
        ...
        error:
          g.error && g.error !== 'RATE_LIMIT' && g.error !== 'PAYMENT_REQUIRED'
            ? g.error
            : null,
      }));
