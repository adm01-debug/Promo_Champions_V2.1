...
function healthFromBalance(
  gini: number,
  unownedRatio: number,
  underservedRatio: number
): string {
...
Deno.serve(withRequestId('territory-optimization', async (req, _ctx) => {
  ...
  try {
    ...
    const [{ data: territories, error: terrErr }, { data: people }, { data: sales }] =
      await Promise.all([
        supabase
          .from('sales_territories')
          .select(
            'id,territory_name,territory_type,current_owner_id,total_revenue,total_deals,is_contested'
          ),
    ...
    (people ?? []).forEach((p: { id: string; name: string }) =>
    ...
    const analyses: TerritoryAnalysis[] = terrs.map(t => {
      const ownerSales = t.current_owner_id
        ? ownerRevenue.get(t.current_owner_id)
        : undefined;
      ...
      else if (coverage < 0.25 && Number(t.total_revenue) > medianRevenue * 0.5)
        status = 'underserved';
      ...
      return {
        ...
        owner_name: t.current_owner_id
          ? (peopleMap.get(t.current_owner_id) ?? null)
          : null,
    ...
    ownerTerrCount.forEach((list, _ownerId) => {
      if (list.length >= 3) {
        list.forEach(a => {
    ...
    const loads: SalespersonLoad[] = Array.from(ownerTerrCount.entries()).map(
      ([id, list]) => {
    ...
    const avgCoverage = total
      ? analyses.reduce((s, a) => s + a.coverage_score, 0) / total
      : 0;
    const potentialLost = analyses
      .filter(
        a =>
          a.status === 'underserved' || a.status === 'unowned' || a.status === 'stagnant'
      )
      .reduce((s, a) => s + Math.max(0, a.potential_revenue - a.recent_revenue), 0);

    const balance = gini(loads.map(l => l.territories_count));
    const health = healthFromBalance(
      balance,
      total ? unowned / total : 0,
      total ? underserved / total : 0
    );
