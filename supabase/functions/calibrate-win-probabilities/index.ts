...
Deno.serve(async req => {
  ...
  try {
    ...
    const { data: closed, error: e1 } = await supabase
      .from('sales')
      .select(
        'id, status, stage, probability, salesperson_id, segment, category, source, created_at, updated_at'
      )
    ...
    const bucketRows: CalibrationBucket[] = [];
    for (const [key, deals] of groups.entries()) {
      ...
      for (const b of BUCKETS) {
        ...
        const wins = inBucket.filter(d =>
          ['completed', 'won'].includes(String(d.status).toLowerCase())
        ).length;
    ...
    const calibrations: DealCalibration[] = [];
    for (const s of open ?? []) {
      const stage = (s.stage ?? s.status ?? 'unknown').toString().toLowerCase();
      const segment = (s.segment ?? s.category ?? 'all').toString();
      ...
      const bucket =
        bucketMap.get(`${stage}::${segment}::${b.min}`) ??
      ...
      const flag =
        delta < -15 ? 'overconfident' : delta > 15 ? 'underconfident' : 'aligned';
