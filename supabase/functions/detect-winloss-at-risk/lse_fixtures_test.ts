...
for (const family of Object.keys(DEAL_HISTORY_LSE_FIXTURES) as DealHistoryLSEFamily[]) {
  ...
  for (const intensity of LSE_INTENSITIES) {
    ...
    Deno.test(`LSE[${family}.${intensity}] — ${c.name}`, () => {
      ...
      const { risk_score, reasons, suggested_action, breakdown, matched_pattern } =
        result;

      assert(
        risk_score >= (c.expect.minScore ?? 0) &&
          risk_score <= (c.expect.maxScore ?? 100),
      ...
      if (c.expect.matchedPatternLabelIncludes) {
        const m = matchesPatternFamily(
          matched_pattern,
          c.expect.matchedPatternLabelIncludes
        );
        assert(
          m.ok,
...
Deno.test(
  'LSE: catalog shape — pricing/negotiation/churn each expose light/strong/edge',
  () => {
    assertEquals(Object.keys(DEAL_HISTORY_LSE_FIXTURES).sort(), [
      'churn',
      'negotiation',
      'pricing',
    ]);
    for (const family of Object.keys(
      DEAL_HISTORY_LSE_FIXTURES
    ) as DealHistoryLSEFamily[]) {
      const group = DEAL_HISTORY_LSE_FIXTURES[family];
      assert(
        group.dominantPatternLabel.length > 0,
        `${family} missing dominantPatternLabel`
      );
      for (const intensity of LSE_INTENSITIES) {
        assert(group.cases[intensity], `${family}.${intensity} missing`);
      }
      assertEquals(
        group.cases.edge.expect.included,
        false,
        `${family}.edge must be excluded`
      );
      assertEquals(
        group.cases.light.expect.included,
        true,
        `${family}.light must be included`
      );
      assertEquals(
        group.cases.strong.expect.included,
        true,
        `${family}.strong must be included`
      );
    }
  }
);
...
Deno.test(
  'LSE meta-coverage: every group has ≥1 included AND ≥1 excluded/edge case',
  () => {
    const gaps: Array<{
...
Deno.test(
  'LSE meta-coverage: each group covers expected matched_pattern_type set (declared + runtime)',
  () => {
    const gaps: string[] = [];
      ...
      if (extra.length > 0 || missing.length > 0) {
        gaps.push(
          `[${family}] declared patternTypeOneOf union mismatch:\n` +
            ...
            (extra.length > 0
              ? `      unexpected extras: ${JSON.stringify(extra)}\n`
              : '') +
            (missing.length > 0
              ? `      missing from fixtures: ${JSON.stringify(missing)}`
              : '')
        );
      }
    }
    ...
    if (gaps.length > 0) {
      throw new Error(
        `LSE matched_pattern_type coverage gaps:\n  ✗ ${gaps.join('\n  ✗ ')}`
      );
    }
    assertEquals(gaps, []);
  }
