...
for (const scenario of SCENARIOS) {
  Deno.test(`scenario: ${scenario.name} — ${scenario.story}`, () => {
    ...
    assert(
      ...
      `${scenario.name}: score out of [0,100]`
    );
    ...
    if (scenario.expect.actionIncludes !== undefined) {
      const evalRes = evaluateActionIncludes(
        r.suggested_action,
    ...
    assert(
      r.suggested_action.length > 10,
      `${scenario.name}: suggested_action too short`
    );
...
Deno.test('scenarios: computeAtRiskDeals sorts desc, filters <40, respects limit', () => {
  ...
  const out = computeAtRiskDeals(allDeals, LOSS_PATTERNS_REALISTIC, NOW, {
    threshold: 40,
    limit: 50,
  });
  ...
  for (const r of out) {
    assert(
      r.risk_score >= 40,
      `included deal ${r.sale_id} has score ${r.risk_score} < 40`
    );
  }
  ...
  // Limit is respected
  const limited = computeAtRiskDeals(allDeals, LOSS_PATTERNS_REALISTIC, NOW, {
    threshold: 40,
    limit: 3,
  });
...
Deno.test(
  'scenarios: every included scenario declares a meaningful actionIncludes (anti-regression)',
  () => {
    ...
    const offenders = SCENARIOS.filter(s => s.expect.included)
      .map(s => {
      ...
      diff.push({
        ...
        severity: r
          ? (r.breakdown.severity ??
            severityFromScore(r.risk_score, r.breakdown.matched_confidence))
          : null,
...
Deno.test(
  'fixtures table: every included scenario respects minScore/maxScore + 0–100',
  () => {
    type ScoreFailure = {
      name: string;
      score: number | null;
...
Deno.test(
  'fixtures table: excluded scenarios stay below threshold even at threshold=0',
  () => {
    type LeakFailure = {
      name: string;
      score: number;
...
Deno.test('fixtures table: SUMMARY — included/failed counts by assert category', () => {
  ...
  for (const s of SCENARIOS) {
    ...
    if (gotIncluded !== s.expect.included) {
      failures.includedFlag.push(
        `${s.name} (expected=${s.expect.included}, got=${gotIncluded})`
      );
    }
    ...
    if (s.expect.included && r) {
      ...
      if (
        (typeof min === 'number' && r.risk_score < min) ||
        (typeof max === 'number' && r.risk_score > max) ||
        r.risk_score < 0 ||
        r.risk_score > 100
      ) {
        failures.scoreBand.push(
          `${s.name} (score=${r.risk_score}, band=[${min ?? '—'},${max ?? '—'}])`
        );
      }
      const reasonNeedles = s.expect.reasonsInclude ?? [];
      const reasonMissing = reasonNeedles.filter(
      ...
      if (s.expect.actionIncludes !== undefined) {
        ...
        if (!ev.ok) {
          failures.action.push(
            `${s.name} (${ev.reason}; missing=${JSON.stringify(ev.missing)})`
          );
        }
      }
    }
...
function allowedBucketsForBand(
...
Deno.test(
  "severity alignment: excluded scenarios resolve to 'low' (or null) at threshold=0",
  () => {
...
Deno.test(
  'severity alignment: included scenarios land in a bucket reachable from their score band',
  () => {
    type SevFailure = {
      name: string;
      score: number;
...
Deno.test(
  'severity alignment: per-scenario coherence check (one assertion per scenario)',
  async t => {
    for (const s of SCENARIOS) {
      await t.step(`${s.name} → ${s.expect.included ? 'included' : 'excluded'}`, () => {
        ...
        const allowed = allowedBucketsForBand(s.expect.minScore, s.expect.maxScore);
        assert(
          allowed.includes(bucket) ||
            (allowed.includes('critical') && bucket === 'high' && conf < 0.7),
