...
function buildCaseReport(
  ...
  const report: CaseReport = {
    ...
    name: c.name,
    expected: {
      included: c.expect.included,
      minScore: c.expect.minScore,
      maxScore: c.expect.maxScore,
    },
    got: {
  ...
  if (
    c.expect.patternTypeOneOf?.length &&
    !c.expect.patternTypeOneOf.includes(r.breakdown.matched_pattern_type)
  ) {
    failures.push(
  ...
  const expectedLabel =
    c.expect.matchedPatternLabelIncludes ?? group.dominantPatternLabel;
  if (expectedLabel) {
  ...
  for (const needle of c.expect.reasonsInclude ?? []) {
    ...
    if (!hit) {
      failures.push(
        `reason needle "${needle}" not found in ${JSON.stringify(r.reasons)}`
      );
    }
  }
...
for (const family of FAMILIES) {
  ...
  Deno.test(
    `history catalog: ${family} — "${group.theme}" (dominant: "${group.dominantPatternLabel}")`,
    () => {
      const reports = validateGroup(family, group);
      const failedCases = reports.filter(r => r.failures.length > 0);
      if (failedCases.length > 0) {
        const flat = failedCases.flatMap(r =>
          r.failures.map(f => `[${r.family}/${r.name}] ${f}`)
        );
...
Deno.test(
  'history catalog: excluded cases stay below threshold even at threshold=0',
  () => {
    ...
    for (const family of FAMILIES) {
      ...
      for (const c of group.cases) {
        ...
        if (score >= 40) {
          violations.push(
            `[${family}/${c.name}] excluded but score ${score} >= 40 (would leak in)`
          );
        }
      }
    }
...
Deno.test(
  'history catalog: every included case declares minScore, maxScore and matchedPatternLabelIncludes (or inherits group default)',
  () => {
    ...
    for (const family of FAMILIES) {
      ...
      for (const c of group.cases) {
        if (!c.expect.included) continue;
        if (typeof c.expect.minScore !== 'number')
          missing.push(`[${family}/${c.name}] missing minScore`);
        if (typeof c.expect.maxScore !== 'number')
          missing.push(`[${family}/${c.name}] missing maxScore`);
        ...
        if (!label)
          missing.push(
            `[${family}/${c.name}] missing matchedPatternLabelIncludes (no group default)`
          );
      }
    }
...
Deno.test(
  'history catalog: every included case declares a meaningful actionIncludes (anti-regression)',
  () => {
    ...
    for (const family of FAMILIES) {
      ...
      for (const c of group.cases) {
        ...
        if (!hasMeaningfulActionIncludes(a)) {
          offenders.push(
            `[${family}/${c.name}] empty actionIncludes (got ${JSON.stringify(a)})`
          );
        }
      }
    }
...
Deno.test(
  '_testHelpers: hasMeaningfulActionIncludes rejects undefined / [] / empty strings',
  () => {
