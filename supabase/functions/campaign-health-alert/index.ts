  ...
  try {
    ...
    for (const d of drafts) {
      ...
      if (d.sent_at) {
        ...
        if (
          Number.isFinite(ts) &&
          (bucket.lastSentAt === null || ts > bucket.lastSentAt)
        ) {
          bucket.lastSentAt = ts;
        }
        const email = String(d.recipient_email ?? '')
          .trim()
          .toLowerCase();
        const optTs = email ? optOutAt.get(email) : undefined;
        // Só conta descadastro posterior ao envio (atribuição correta).
        if (optTs !== undefined && Number.isFinite(ts) && optTs >= ts)
          bucket.optedOut += 1;
      } else if (d.error) {
        bucket.failed += 1;
      } else {
    ...
    const cooldownSince = new Date(
      now.getTime() - COOLDOWN_HOURS * 3600_000
    ).toISOString();
    ...
    const dedupeBucket = new Date(
      Math.floor(now.getTime() / cooldownMs) * cooldownMs
    ).toISOString();
    const alertRows = toInsert.map(alert => ({ ...alert, dedupe_bucket: dedupeBucket }));
    const { data: insertedAlerts, error: insertError } = await supabase
      .from('campaign_health_alerts')
      .upsert(alertRows as never, {
        onConflict: 'job_id,alert_type,dedupe_bucket',
    ...
    const insertedKeys = new Set(
      (insertedAlerts ?? []).map(alert => `${alert.job_id}:${alert.alert_type}`)
    );
    const notifications = toInsert
      .filter(a => insertedKeys.has(`${a.job_id}:${a.alert_type}`))
      .map(a => ({
        user_id: a.owner_id,
