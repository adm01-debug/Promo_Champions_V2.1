...
async function persistInvocationAudit(
  ...
  if (error) {
    jlog('error', {
      msg: 'invocation_audit_persist_failed',
  ...
  try {
    ...
    if (claimsErr || !claimsData?.claims?.sub) {
      jlog('warn', {
        ...
        requestId,
        ...(claimsErr ? describeError(claimsErr) : {}),
      });
      return jsonResponse({ error: 'Unauthorized', requestId }, 401, requestId);
    }
      ...
      return jsonResponse(
        {
          error: 'invalid_input',
          message:
            'Provide exactly one of dead_letter_ids or delivery_ids (1–50 valid UUIDs).',
    ...
    const ids: string[] = (parsed.data.dead_letter_ids ??
      parsed.data.delivery_ids) as string[];
