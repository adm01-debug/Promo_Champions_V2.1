...
const DISCOUNT_BUCKETS = [
  ...
  { label: '1-10%', min: 0.001, max: 0.1 },
  { label: '11-20%', min: 0.1, max: 0.2 },
  { label: '21-30%', min: 0.2, max: 0.3 },
  { label: '>30%', min: 0.3, max: 1 },
];
...
function pricingHealth(avgDiscount: number, alertRatio: number): string {
  ...
  if (avgDiscount < 0.2 && alertRatio < 0.3) return 'warning';
  return 'critical';
}
  ...
  withRequestId('pricing-intelligence', async (req, _ctx) => {
    ...
    try {
      ...
      const wonRows = rows.filter(
        r => r.status === 'won' || r.status === 'completed' || r.status === 'Fechado'
      );
      ...
      let revenueLost = 0;
      let alertedDeals = 0;
      const distribution = DISCOUNT_BUCKETS.map(b => ({
        label: b.label,
        count: 0,
        revenue: 0,
      }));
      ...
      const topDiscounters: SalespersonAgg[] = Array.from(perSalesperson.entries())
        .map(([id, v]) => {
          ...
          const avgDisc =
            v.discounts.reduce((s, d) => s + d, 0) / Math.max(1, v.discounts.length);
          const refSum = v.deals.reduce(
            (s, d) => s + (productRefPrice.get(d.product_name) ?? Number(d.amount)),
            0
          );
          return {
            salesperson_id: id,
      ...
      wonRows.forEach(r =>
        wonProducts.set(r.product_name, (wonProducts.get(r.product_name) ?? 0) + 1)
      );
