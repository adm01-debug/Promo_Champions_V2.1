...
- **Health monitor** (`winloss-webhook-health-monitor`): grava em `winloss_webhook_alerts` com `request_id`/`subscription_id` top-level + `monitor_request_id` em `details`. Três tipos:
  - `consecutive_failures` / `high_retry_rate` — suprimidos por kind, janela `SUPPRESS_MINUTES`
  - `attempts_exhausted` — todas as `MAX_ATTEMPTS` (3) tentativas de um mesmo `request_id` falharam; suprimido por request_id
